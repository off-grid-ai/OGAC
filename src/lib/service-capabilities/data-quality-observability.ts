...
const OBSERVABILITY_AUDITS: readonly ServiceCapabilityAudit[] = [
  ...
  },
  {
    ...
    items: [
      capability('faithfulness', 'Faithfulness', 'Judge whether the answer is supported by retrieved context.', '/solutions/quality/evaluators', 'Manage evaluators', 'Retained per-metric evidence + governed judge identity are now persisted on each run; production-scale multi-sample runs still want the async/durable eval path.', [
