...
const ENTERPRISE_SOURCE_AUDITS: readonly ServiceCapabilityAudit[] = [
  ...
  stale({
    ...
    summary:
      'The Salesforce-style REST fixture supplies customer and cross-sell context. Governed task and opportunity writes are live through Apps with human approval, zero-mutation shadow execution, idempotent replay, and signed receipts. Immutable source identity and sync/webhook breadth remain gaps.',
    items: [
      ...
      capability('write-sync-webhooks', 'Writes, pagination, sync, and webhooks', 'Create/update records, traverse pagination, perform incremental sync, and receive changes.', '/build/studio/new', 'Build governed CRM action', 'Extend the live governed action seam with typed pagination, incremental checkpoints, rate-limit handling, webhook ingestion, and the remaining audited CRM CRUD lifecycle.', [
        'yes', 'Salesforce-style REST systems provide CRUD, pagination, incremental APIs, and webhooks/events.', 'partial', 'Existing bounded CRM adapters and the App action runtime execute tenant-scoped, allowlisted task/opportunity mutations with exact human approval, runtime-derived idempotency, signed receipts, and fail-closed internal endpoints. Pagination, sync, and webhooks are not integrated.', 'partial', 'The nontechnical App builder configures a CRM action, a prior human reviewer, an impact preview, and retained execution evidence. CRM sync and webhook controls remain absent.', 'partial', 'Console SHA 16fa96443c79 completed Bharat App run apprun_5e715894: awaiting human approval, one approved CRM task mutation (count 0→1), retained reviewer/impact/signed receipt, duplicate approval rejected 409, and provider replay returned 200 without a second task. Shadow run apprun_71da60a4 completed with CRM count unchanged at 1.',
      ]),
    ],
  }),
];
