...
const ENTERPRISE_SOURCE_AUDITS: readonly ServiceCapabilityAudit[] = [
  ...
  stale({
    ...
    summary:
      'The Salesforce-style REST fixture supplies customer and cross-sell context. Governed task and opportunity writes are code-wired through Apps; exact live deployment proof, immutable source identity, and sync/webhook breadth remain gaps.',
    items: [
      ...
      capability('write-sync-webhooks', 'Writes, pagination, sync, and webhooks', 'Create/update records, traverse pagination, perform incremental sync, and receive changes.', '/build/studio/new', 'Build governed CRM action', 'Deploy an exact Console SHA and prove one bank maker-checker CRM mutation plus idempotent replay against the live fixture. Typed pagination, incremental sync, rate-limit handling, webhooks, and broad CRM CRUD remain outside this narrow action seam.', [
        'yes', 'Salesforce-style REST systems provide CRUD, pagination, incremental APIs, and webhooks/events.', 'partial', 'Existing bounded CRM adapters and the App action runtime execute tenant-scoped, allowlisted task/opportunity mutations with exact human approval, runtime-derived idempotency, signed receipts, and fail-closed internal endpoints. Pagination, sync, and webhooks are not integrated.', 'partial', 'The nontechnical App builder configures a CRM action, a prior human reviewer, an impact preview, and retained execution evidence. CRM sync and webhook controls remain absent.', 'no', 'Source and integration tests prove approval gating, shadow-mode interception, execute/replay contracts, and receipt persistence, but no exact deployed bank workflow has changed CRM through the Console.',
      ]),
    ],
  }),
];
