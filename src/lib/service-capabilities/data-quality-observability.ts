...
const OBSERVABILITY_AUDITS: readonly ServiceCapabilityAudit[] = [
  ...
  },
  {
    ...
    items: [
      ...
      capability('projects-history-monitoring', 'Projects, report history, and monitoring', 'Persist reports, compare runs, and operate monitoring projects.', '/solutions/quality/drift', 'Open current drift view', 'The integration is stateless. Add a deliberate system of record or explicitly keep Evidently project history out of scope.', [
        'yes', 'Evidently provides report and monitoring workflows.', 'no', 'The adapter stores no Evidently project or report identifier.', 'no', 'The console shows current normalized drift only.', 'no', 'No workflow reads Evidently history or monitoring state.',
      ]),
    ],
  },
  {
    ...
    auditStateEvidence: null,
    summary:
      'The air-gap-safe sidecar runs five real Ragas metrics through the on-prem gateway. Engine attribution and persisted per-metric runs need stronger fleet proof.',
    items: [
      capability('faithfulness', 'Faithfulness', 'Judge whether the answer is supported by retrieved context.', '/solutions/quality/evaluators', 'Manage evaluators', 'Persist sidecar-attributed per-metric evidence and the judge model/deployment identity for every run.', [
        'yes', 'Ragas 0.2.6 provides Faithfulness.', 'yes', 'The sidecar builds Faithfulness and the eval runner requests and parses its normalized score.', 'yes', 'The evaluator catalog and run surfaces expose faithfulness.', 'partial', 'Ragas workflows are wired, but current fleet evidence proves health/evaluation APIs rather than a retained service-attributed production run.',
      ]),
      capability('answer-relevancy', 'Answer relevancy', 'Score whether an answer addresses the question.', '/solutions/quality/evaluators', 'Manage evaluators', 'Retain service-attributed run evidence and surface omitted-metric degradation instead of only falling back.', [
        'yes', 'Ragas 0.2.6 provides ResponseRelevancy.', 'yes', 'The sidecar maps answer_relevancy and the runner can request it.', 'yes', 'Answer relevancy is available in the evaluator catalog.', 'partial', 'The workflow can run it, but no retained fleet result proves the engine path.',
      ]),
  ...
  },
  {
    ...
    items: [
      capability('metrics-query', 'Metrics query and range analysis', 'Query current and historical time series with PromQL-compatible expressions.', '/insights/platform', 'Open platform metrics', 'Queries are wired, but zero application series means the cards do not yet prove workload telemetry. Add producer coverage and freshness evidence.', [
        'yes', 'VictoriaMetrics 1.106.1 exposes Prometheus-compatible instant and range query APIs.', 'yes', 'The metrics adapter runs bounded queries for platform and collector signals.', 'yes', 'Platform insights exposes metric cards and time ranges.', 'no', 'The 2026-07-20 fleet record reports zero application series.',
      ]),
      ...
      capability('alerts-recording-rules', 'Alerts and recording rules', 'Create rule groups, evaluate alerts, and inspect firing state.', '/insights/platform', 'Inspect platform metrics', 'No vmalert rule lifecycle or alert-state adapter is registered. First-party alerts are not evidence of VictoriaMetrics rule integration.', [
        'yes', 'The VictoriaMetrics stack supports vmalert recording and alert rules.', 'no', 'No vmalert adapter or configured rule API is integrated.', 'partial', 'Platform alerts exist without VictoriaMetrics rule provenance.', 'no', 'No production workflow consumes VictoriaMetrics-native alert state.',
      ]),
  ...
  },
  {
    ...
    items: [
      capability('log-query', 'Log search and field filtering', 'Search stored logs by time, text, stream, and structured fields.', '/operations/health/logs', 'Open platform logs', '', [
        'yes', 'VictoriaLogs 1.3.2 exposes LogsQL query and field APIs.', 'yes', 'The log adapter runs bounded time-window queries and normalizes records.', 'yes', 'Platform Logs exposes search, filters, and detail.', 'yes', 'The fleet record confirms live g5 logs are queryable.',
      ]),
      ...
      capability('retention-alerts-tenancy', 'Retention, alerts, and tenant operations', 'Manage retention, alert rules, tenant boundaries, and storage lifecycle.', '/operations/services/victorialogs', 'Inspect VictoriaLogs', 'No lifecycle or rule adapter is exposed. Keep these deployment-owned until tenant-safe configuration and rollback are designed.', [
        'yes', 'VictoriaLogs supports retention controls and integrations for alert evaluation.', 'no', 'No retention, alert-rule, or tenant-management adapter exists.', 'no', 'No corresponding management UI exists.', 'no', 'No production workflow manages these controls through the console.',
      ]),
    ],
  },
  stale({
  ...
  }),
  {
    ...
    items: [
      ...
      capability('trace-search-detail', 'Trace search and span detail', 'Search by service, operation, tags, duration, and time and inspect span trees.', '/operations/health/traces', 'Explore traces', '', [
        'yes', 'Jaeger query APIs expose services, operations, traces, and span data.', 'yes', 'The trace adapter queries and normalizes real Jaeger results.', 'yes', 'Trace explorer exposes URL-driven trace detail.', 'yes', 'Operators can investigate the verified fleet trace path.',
      ]),
      ...
      capability('storage-retention-admin', 'Storage, retention, and service administration', 'Configure storage backends, retention, archive, sampling, and collector/query topology.', '/operations/services/jaeger', 'Inspect Jaeger', 'The all-in-one deployment is configured outside the console. Keep lifecycle deployment-owned or add guarded configuration and backup evidence.', [
        'yes', 'Jaeger supports pluggable storage and deployment topology controls.', 'partial', 'Service health and query clients exist, but storage/config lifecycle does not.', 'no', 'No retention, storage, or topology management UI exists.', 'no', 'No console workflow changes Jaeger service state.',
      ]),
    ],
  },
];
...
const ENTERPRISE_SOURCE_AUDITS: readonly ServiceCapabilityAudit[] = [
  ...
  stale({
    ...
    summary:
      'The Salesforce-style REST fixture is queryable through the REST connector and supplies customer/cross-sell context. Version identity and write semantics remain gaps.',
    items: [
      ...
      capability('write-sync-webhooks', 'Writes, pagination, sync, and webhooks', 'Create/update records, traverse pagination, perform incremental sync, and receive changes.', '/data/flows/replication', 'Inspect replication', 'The connector is read-only and treats one JSON response generically. Add typed pagination, write policy, incremental state, rate limits, and webhook verification.', [
        'yes', 'Salesforce-style REST systems provide CRUD, pagination, incremental APIs, and webhooks/events.', 'partial', 'The adapter supports bounded GET reads only.', 'partial', 'Resources can be read; write and sync controls are absent.', 'no', 'No production CRM write, incremental sync, or webhook workflow exists.',
      ]),
    ],
  }),
];
