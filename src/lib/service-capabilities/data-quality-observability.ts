  ...
  },
  {
    ...
    items: [
      ...
      capability('produce-records', 'Produce JSON records', 'Publish keyed or unkeyed JSON through the native Kafka protocol.', '/operations/services/streaming?manage=topics', 'Open producer', 'The producer is an operator workbench and proof helper; bind it to governed pipeline outputs before calling arbitrary production publishing integrated.', [
        'yes', 'Redpanda accepts Kafka-compatible keyed records.', 'yes', 'The adapter validates the topic and JSON and produces without auto-create.', 'yes', 'The Topics view exposes a JSON producer.', 'yes', 'CLOSED 2026-08-04 — a governed pipeline output now uses this adapter. A "topic" sink registered in SINK_REGISTRY publishes a run outcome through the SAME sequence every other sink runs (egress leash → PII mask → deliver → honest record); air-gapped, like the on-prem WhatsApp gateway, and never auto-creating a topic. Proven live: honest degrade when no topic is named; the broker\'s own refusal for an unknown topic; publish acknowledged partition 0 / offset 0; the record consumed back with its run-id key and full JSON. Checked against the REAL pl_seed_org_bharat contract, masking is required and the raw PAN becomes [PAN] before publication — a topic is durable and multi-consumer, so internal is not unprotected.',
      ]),
...
const OBSERVABILITY_AUDITS: readonly ServiceCapabilityAudit[] = [
  ...
  stale({
    ...
    items: [
      ...
      capability('self-telemetry', 'Collector throughput, errors, and queues', 'Inspect accepted signals, exporter failures, batches, and queue depth.', '/operations/health/metrics', 'Inspect collector metrics', 'CLOSED 2026-08-04 for self-metrics reaching the store. Remaining: an ALERT that consumes them — vmalert rule authoring is tracked on the VictoriaMetrics alerts item, so the counters are queryable and not yet alerted on.', [
        'yes', 'The collector emits receiver, processor, and exporter metrics.', 'yes', 'A verified self-telemetry export path now exists: service.telemetry.metrics with a periodic OTLP reader to VictoriaMetrics. GOTCHA worth keeping — the telemetry reader posts to the endpoint EXACTLY as written while the otlp_http pipeline exporter appends /v1/metrics itself, so the same base URL works for one and 400s for the other ("unsupported path requested: /opentelemetry"). The reader needs the full /opentelemetry/v1/metrics.', 'yes', 'Platform Health has collector metric queries and they now return values.', 'yes', 'Proven live 2026-08-04: VictoriaMetrics holds otelcol_receiver_accepted_spans / _metric_points, otelcol_exporter_sent_spans / _metric_points, otelcol_exporter_queue_size / _capacity, otelcol_processor_memory_limiter_accepted_*, plus process CPU/RSS/uptime. Accepted and exported are both present, so the accepted-vs-exported correlation the ingest item wanted is queryable.'
      ]),
    ],
  }),
  {
