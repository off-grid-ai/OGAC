...
const DATA_AUDITS: readonly ServiceCapabilityAudit[] = [
  ...
  },
  {
    ...
    items: [
      ...
      capability('payload-filtering', 'Payload schema and filtering', 'Filter retrieval by tenant, source, and metadata and manage payload indexes.', '/data/knowledge/indexes', 'Inspect index metadata', 'CLOSED 2026-08-04. The finding that made it worth doing: the deployed offgrid-brain collection had NO payload index at all, while EVERY governed retrieval filters on org_id — the tenant isolation boundary — so Qdrant answered that filter by scanning. Invisible at three points; the first thing to fall over as a corpus grows.', [
