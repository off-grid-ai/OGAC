...
const DATA_AUDITS: readonly ServiceCapabilityAudit[] = [
  ...
  },
  {
    ...
    items: [
      ...
      capability('payload-filtering', 'Payload schema and filtering', 'Filter retrieval by tenant, source, and metadata and manage payload indexes.', '/data/knowledge/indexes', 'Inspect index metadata', '', [
        'yes', 'Qdrant supports payload filters and payload indexes.',
        'yes', 'listPayloadIndexes / createPayloadIndex / dropPayloadIndex on the snapshots port (create uses wait=true so a caller that then lists sees it — an async create makes "prove it exists" a race, and an unverifiable create is not a managed index). recommendPayloadIndexes is pure and only ever suggests fields the platform ACTUALLY filters on, since an index costs memory and write time; validateIndexRequest refuses a field name before it reaches a REST path. 9 unit tests built on the real payload_schema shapes the deployment returned before and after.',
        'yes', 'PayloadIndexManager on the collection detail page beside snapshots: current indexes with their type and indexed count, one-click creation of a recommendation WITH the type the rule already decided (retyping "keyword" is how the field that matters most ends up the wrong type), and drop. Write routes return the RE-READ state so the surface shows what the store actually has.',
        'yes', 'CLOSED 2026-08-04. The finding that made it worth doing: the deployed offgrid-brain collection had NO payload index at all, while EVERY governed retrieval filters on org_id — the tenant isolation boundary — so Qdrant answered that filter by scanning. Invisible at three points; the first thing to fall over as a corpus grows. Verified live on the fleet 2026-07-21: a governed Qdrant retrieval applied filters=tenant:org_id/match + acl:document_acl/grants, attributed to provider=qdrant. And 2026-08-04, the index lifecycle end to end on the live store: BEFORE indexes=[] recommending org_id:keyword + text:text; after creating both, indexes=[org_id keyword (3 points), text text (3 points)] and recommendations empty ("Every field this platform filters on is indexed"). Full CRUD round-trip through the admin route: GET → DELETE text (the recommendation returns with its reason) → POST (it clears) → a malformed field name answers 400.',
      ]),
  ...
  },
  {
    ...
    items: [
      capability('tables-schema', 'Table and schema lifecycle', 'Create vector tables, inspect schema, and delete local indexes.', '/data/knowledge/indexes', 'Manage indexes', 'THIS DEPLOYMENT DOES NOT SELECT LANCEDB. OFFGRID_ADAPTER_RETRIEVAL=qdrant, so the Brain reads Qdrant and no live LanceDB retrieval exists here to attribute. The attribution code is correct and unit-proven; proving it end to end needs a deployment that selects lancedb. That is a configuration boundary, not a code gap.', [
        ...
        'partial', 'ATTRIBUTION FIXED AND UNIT-PROVEN 2026-08-05: collection was Qdrant-only and hard-coded to null for every other provider, so a LanceDB retrieval recorded WHICH provider ran but not WHAT it read - which made \"the provider-neutral port selected LanceDB\" unfalsifiable. It now names the table. But selected LanceDB execution is still not service-attributed live, and is NOT claimed as such: a real retrieval on this box returned providerId=qdrant collection=offgrid-brain, because the deployment selects Qdrant. Measured, not assumed.',
      ]),
  ...
  },
  {
    ...
    items: [
      ...
      capability('lifecycle-versioning', 'Lifecycle and versioning', 'Bound how long files are kept, per destination a governed run writes to.', '/data/lake', 'Manage bucket retention', 'THE STORE CANNOT HOLD A BFSI WINDOW: an expiry over 255 days wraps to (days mod 256) — 3650 becomes 66 — so a 7- or 10-year window cannot be enforced here at all (G-209, owner: fleet repo). Our side refuses to write such a rule rather than schedule early deletion, so the gap is honest, but it is a gap. Versioning and object-lock remain unimplemented.', [
        'partial', 'The S3 lifecycle API is exposed, but this deployment truncates the expiry day count to a single byte: 30 days round-trips as 30, 365 reads back as 109, 3650 reads back as 66. Measured live 2026-08-05 with our own XML and parser verified correct independently. The sector needs 2555 and 3650, so the required windows are exactly the ones that wrap.',
        'yes', 'The storage port reads, writes and CLEARS bucket lifecycle rules (verified live; clearing had to become a DELETE because the store accepts an empty PUT and ignores it, and the response parser had to scope <Days> to <Expiration> or a cold-storage transition read back as a 1-day EXPIRY). applyLakeRetention derives destinations from the apps that write there, merges rather than replaces so an operator rule is never dropped, re-reads to report what the bucket actually holds, and REFUSES any window the store would silently shorten.',
        'yes', 'RetentionPanel on /data/lake shows the window per destination in days-to-keep, add and remove, written for the compliance reader who answers for the period. A failed read says the rules are UNKNOWN rather than "nothing is deleted on a schedule".',
        'yes', 'PROVEN LIVE 2026-08-05 through the SAME sweep as app runs and indexed text, not a second surface: lake_objects is a retainable class, a 200-day policy applied and the sweep filed 1/1 destinations keep files 200 days (remaining 0), re-applying was a no-op, and an operator rule on another prefix survived. A 3650-day policy was REFUSED with the real number named and the bucket left untouched. It also caught a governance defect: destinations were derived from listApps, which hides [autotest] apps for presentation — a presentation filter reaching a compliance calculation — now listAppsForGovernance. docs/evidence/2026-08-05-lake-retention.md',
      ]),
      capability('topology-repair', 'Volume topology, replication, and repair', 'Show how many copies of each file exist, where they live, and how much room is left.', '/data/lake', 'Open object store', 'Rebalancing, volume reassignment and EC repair remain native operations — they are destructive, and exposing them before ownership and rollback boundaries exist would be worse than not having them. What is closed is knowing the durability and capacity; ACTING on them is not.', [
        'yes', 'SeaweedFS 3.80 exposes topology, cluster and volume status on the master.',
        'yes', 'object-topology-read.ts reads the master status; object-topology.ts turns it into a durability answer. Replication is PARSED from the three-digit code rather than matched against known values, so an unfamiliar code reads as UNKNOWN and never as safe; the WEAKEST replication across collections governs, so one exposed collection cannot hide behind a replicated neighbour; and erasure-coded shards count as redundancy the replication code does not show. A failed read returns no view at all — a zeroed one would render as a store with no redundancy, alarming for the wrong reason.',
        'yes', 'DurabilityPanel leads /data/lake, answering "if a disk fails, what happens to these files?" instead of a liveness tick. Capacity says what running out actually causes: new files REFUSED while everything still reports healthy.',
        'partial', 'OBSERVING is bound; ACTING is not, and the row name says repair. PROVEN LIVE 2026-08-05 on the deployment, and the answer is uncomfortable, which is the point: replication is 000, so there is ONE copy of every file and losing a disk loses it. 1 node, 1 rack, 1 site, 21 of 130 slots used, version 30GB 3.80. The panel renders that in red with a needs-attention badge (screenshot verified) rather than a green tick — the console could previously only say the store was UP, which is the least interesting thing about it. The parser is unit-tested against the exact payload the deployment returned.',
      ]),
    ],
  },
  stale({
  ...
  },
  {
    ...
    items: [
      ...
      capability('produce-records', 'Produce JSON records', 'Publish keyed or unkeyed JSON through the native Kafka protocol.', '/operations/services/streaming?manage=topics', 'Open producer', '', [
        'yes', 'Redpanda accepts Kafka-compatible keyed records.', 'yes', 'The adapter validates the topic and JSON and produces without auto-create.', 'yes', 'The Topics view exposes a JSON producer.', 'yes', 'CLOSED 2026-08-04 — a governed pipeline output now uses this adapter. A "topic" sink registered in SINK_REGISTRY publishes a run outcome through the SAME sequence every other sink runs (egress leash → PII mask → deliver → honest record); air-gapped, like the on-prem WhatsApp gateway, and never auto-creating a topic. Proven live: honest degrade when no topic is named; the broker\'s own refusal for an unknown topic; publish acknowledged partition 0 / offset 0; the record consumed back with its run-id key and full JSON. Checked against the REAL pl_seed_org_bharat contract, masking is required and the raw PAN becomes [PAN] before publication — a topic is durable and multi-consumer, so internal is not unprotected. The producer is an operator workbench and proof helper; bind it to governed pipeline outputs before calling arbitrary production publishing integrated.',
      ]),
      ...
      capability('consumer-groups-offsets', 'Consumer groups and offsets', 'Inspect lag, reset offsets, and manage durable groups.', '/operations/services/streaming?manage=consumer', 'Open consumer workbench', 'The consumer workbench still exposes no lag read-back or guarded offset reset; the stream trigger manages its own durable state and does not need them.', [
        'yes', 'Kafka-compatible consumer groups and offsets are available.', 'partial', 'The workbench still creates a temporary group; durable per-app cursors are managed by the stream trigger instead (src/lib/topic-trigger-store.ts).', 'partial', 'The UI accepts a group name but exposes no lag or reset.', 'yes', 'A published app with a stream trigger consumes insurance.claim-events and drives a governed run per record, with a checkpointed cursor advanced only behind durable work. Proven live 2026-08-05 on org_suraksha: 17 historical records not replayed, offset 20 to apprun_483326a5, a rewound cursor redelivered and refused (dup=1, run count unchanged). docs/evidence/2026-08-05-stream-trigger-live.md',
      ]),
...
const OBSERVABILITY_AUDITS: readonly ServiceCapabilityAudit[] = [
  ...
  },
  {
    ...
    items: [
      ...
      capability('prompts-datasets', 'Prompt and dataset lifecycle', 'Version prompts, manage labels, create datasets, and run dataset experiments.', '/insights/ai/langfuse-prompts', 'Manage prompts & datasets', '', [
        ...
        'yes', 'Insights → AI → Prompts/Datasets are list→detail management surfaces: create prompt/version, move labels (production/latest), delete; create dataset, add/delete items, and VIEW experiment runs. Runs are deliberately view-only here — see the note above; authoring a result is not a management action.',
        'yes', 'Fleet-proven live: a text prompt (rm-cross-sell-system v1, labels production+latest, isProduction) and a dataset (cross-sell-eval-set) were created through the console admin routes against the live Langfuse and read back in list + detail. CLOSED 2026-08-04. Read-only experiment RUNS is the CORRECT design, not a gap: a dataset-experiment run is a RESULT produced by an eval job, and letting someone author one by hand would let them manufacture evaluation history — on a platform whose value is that its quality claims are checkable, that is the last thing to build. Triggering a run belongs to the eval surface (which does it), not to a prompt/dataset CRUD screen. Verified live: prompts API 200, datasets return real content (cross-sell-eval-set, golden Q/A pairs), and dataset-run-items is POST-only upstream — Langfuse itself treats runs as written-by-jobs.',
      ]),
  ...
  },
  {
    ...
    items: [
      ...
      capability('data-summary', 'Data summary preset', 'Run a dataset summary over the reference and current windows.', '/solutions/quality/drift', 'Open drift', 'Evidently 0.4.40 ships no DataSummaryPreset — the closest is DataQualityPreset, which is what runs. The console reports that substitution rather than hiding it, so the selection is honest but the named preset is not available upstream.', [
        'partial', 'Evidently 0.4.40 has DataQualityPreset but no DataSummaryPreset; the advertised selection has no upstream implementation to call.',
        'yes', 'The sidecar request model accepts preset and returns preset_applied — the preset REALLY executed, not the one requested. Verified live 2026-08-05 through the console adapter into the rebuilt sidecar.',
        'yes', 'The drift surface offers the preset and the run records which preset actually ran.',
        'yes', 'A drift run receives the result and its attribution: engine=evidently, version 0.4.40, and the note "Evidently ran \\"data_quality\\" (asked for \\"data_summary\\")". The claim matches the work.',
      ]),
      capability('data-quality', 'Data quality preset', 'Run Evidently\'s data-quality preset instead of drift.', '/solutions/quality/drift', 'Open drift', '', [
        'yes', 'Evidently 0.4.40 provides DataQualityPreset.',
        'yes', 'The sidecar constructs the REQUESTED preset — it used to always construct DataDriftPreset and silently discard the selection. Verified live 2026-08-05 through the console adapter into the rebuilt sidecar.',
        'yes', 'The drift surface offers the preset and shows what ran.',
        'yes', 'A drift run with preset=data_quality returns engine=evidently, preset_applied=data_quality, version 0.4.40, recorded on the run attribution.',
      ]),
      capability('psi-method', 'PSI as the drift method', 'Ask for Population Stability Index and have Evidently run it.', '/solutions/quality/drift', 'Open drift', '', [
        'yes', 'Evidently implements PSI as a stat test.',
        'yes', 'method=psi is mapped and forwarded; the sidecar no longer ignores it. The console keeps its own PSI only as an explicit FALLBACK, and a fallback is now reported as engine=first-party-psi rather than wearing Evidently\'s name.',
        'yes', 'The method is selectable and the run states which test ran.',
        'yes', 'Live 2026-08-05: a per-column run returned {"column":"score","method":"PSI","score":14.2504,"threshold":0.1} — Evidently\'s OWN label for the test it executed, which is evidence rather than intent.',
      ]),
      capability('stat-tests', 'Statistical test selection', 'Choose the statistical test used to judge drift.', '/solutions/quality/drift', 'Open drift', '', [
        'yes', 'Evidently implements the advertised tests.',
        'yes', 'Every advertised token maps explicitly to Evidently\'s own, including cramer → cramer_von_mises. An UNKNOWN token is dropped and REPORTED, never silently defaulted — "the method you chose was ignored" is the failure this contract exists to stop.',
        'yes', 'The test is selectable and the run states which one ran.',
        'yes', 'Live 2026-08-05: method=ks produced Evidently\'s label "K-S p_value" with threshold 0.05, and an invented token returned the note "ignored unrecognised method token(s): made_up_test" instead of quietly running the default.',
      ]),
      capability('column-overrides', 'Per-column method overrides', 'Use a different statistical test per column.', '/solutions/quality/drift', 'Open drift', 'The contract and the engine both apply per-column overrides, but the drift workflow that exists today compares a single column (the eval score), so no PRODUCT flow sends two. Closing this needs a multi-column drift source, not more adapter work.', [
        'yes', 'Evidently accepts per_column_stattest.',
        'yes', 'A tabular contract ({columns: {name: {reference, current}}}) plus column_methods reaches Evidently; the override wins over the request-wide method, which is the whole point of an override. The single score-array wrapper that dropped both settings is gone.',
        'yes', 'Per-column selections are offered and the run reports the test applied per column.',
        'partial', 'PROVEN at the contract level live 2026-08-05 — two columns, score=psi and latency=ks, came back as {"score", method "PSI", drifted true, 14.2504} and {"latency", method "K-S p_value", drifted false, 1.0}. But the product drift workflow compares one column, so no product flow exercises it yet.',
      ]),
      capability('projects-history-monitoring', 'Projects, report history, and monitoring', 'Persist reports, compare runs, and operate monitoring projects.', '/solutions/quality/drift-monitoring', 'Open drift monitoring', '', [
        'yes', 'Evidently provides report and monitoring workflows.', 'yes', 'A console-owned SoR (evidently-projects-store: self-migrating drift_projects table) persists monitoring projects and composes report history + trend from the retained drift runs (reused read-only).', 'yes', 'Solutions → Quality → Drift monitoring is a list→detail surface: project CRUD, report-history table (Evidently vs PSI attributed), and a drift-share-over-time trend chart with the breach threshold.', 'yes', 'Fleet-proven live: a project was created→listed→detailed with its trend rendering a real retained run against its threshold line. 2026-08-04: attribution added and exercised — an attributed Evidently run (status=drift, share=1) is scoped to its app and project, and the surface distinguishes a project\'s own history from the org context it falls back to. CLOSED 2026-08-04. The caveat was the whole gap: a project\'s history was the ORG\'s runs keyed by its threshold, so two projects on different datasets showed each other\'s reports. drift_runs now records project_id (and app_id, and dataset) AT RUN TIME, and listDriftRunsForProject returns only attributed runs. Runs predating attribution carry no project_id, so the detail falls back to the org window and SAYS SO — "No run has been attributed to this project yet, so these are the organisation\'s recent runs shown for context" — rather than passing them off as the project\'s history.',
      ]),
    ],
  },
  {
    ...
    items: [
      ...
      capability('context-precision-recall', 'Context precision and recall', 'Measure retrieval relevance and coverage against ground truth.', '/solutions/quality/evaluators', 'Manage retrieval metrics', '', [
        'yes', 'Ragas 0.2.6 provides LLMContextPrecisionWithoutReference and LLMContextRecall.', 'yes', 'The sidecar exposes both metrics, AND the console now validates the requested metric list before calling it. Measured live: asking the sidecar for a metric it does not know makes it run ALL FIVE — each a chain of 30–90s gateway LLM calls — and return them without ever saying the request was unsupported, so a typo silently cost ~5x the compute and answered a question nobody asked. eval-runner now filters against RAGAS_METRICS so an unknown metric is reported as OMITTED rather than replaced by everything.', 'yes', 'Both metrics are selectable in quality tooling.', 'yes', 'Both scored live on a retained golden sample (question + answer + contexts + ground_truth) in 40s: context_precision 1.0, context_recall 1.0 with the supporting context present. And they DISCRIMINATE — the same question with only irrelevant contexts scored 0.0 / 0.0, which is what distinguishes a real judge from a constant. A single 1.0 would not have been evidence. CLOSED 2026-08-04, and proving it surfaced a real defect in the omission half — see the adapter note.',
      ]),
  ...
  },
  {
    ...
    items: [
      capability('metrics-query', 'Metrics query and range analysis', 'Query current and historical time series with PromQL-compatible expressions.', '/operations/health/metrics/explorer', 'Open metric explorer', 'The collector\'s own self-telemetry reaches the store, but zero application series do: no console or workflow metric is emitted, so the query path is proven against infrastructure counters only. Closing it needs the application to emit its own metrics, not another query surface.', [
        'yes', 'VictoriaMetrics 1.106.1 exposes Prometheus-compatible instant and range query APIs.', 'yes', 'The victoriametrics adapter runs bounded instant/range PromQL queries + /label/__name__/values against the live VM (verified: query executes, returns a well-formed empty result because VM is empty).', 'yes', 'Operations → Platform health → Metrics is a live PromQL explorer (query, range, metric-name picker, chart) with a console-owned saved-query panel; saved-query CRUD is fleet-proven live (create→list→delete of "up-probe" in real org-scoped Postgres).', 'no', 'A REAL VALUE read end-to-end 2026-08-04: query=offgrid_audit_events_total returned result[0] with value 1 and labels {action=probe.metric, org=org_bharat, outcome=ok, service.name=offgrid-console}, seriesFetched 1. The query path was always correct; it had nothing to return until the collector could accept metrics. CLOSED 2026-08-04. VM was empty because the collector had NO metrics pipeline (POST /v1/metrics 404), not because scraping was unconfigured — the previous note pointed at the wrong layer. With the pipeline added, a real value now reads end-to-end.',
      ]),
      capability('ingest-remote-write', 'Prometheus remote write and OTLP ingestion', 'Accept application and collector metrics through remote-write or OTLP paths.', '/operations/health/metrics', 'Inspect metrics health', '', [
        'yes', 'VictoriaMetrics accepts Prometheus remote write and supported OTLP ingestion paths.', 'yes', 'The collector\'s metrics pipeline exports OTLP to VM cross-node by mDNS name, and the console produces named samples through emitCounter/emitGauge on the same wire as spans. Sample attribution carries service.name plus the producer\'s own dimensions (action/outcome/org).', 'yes', 'Operations exposes destination health and query results.', 'yes', 'A named producer verified end-to-end 2026-08-04: governed audit events emitted offgrid_audit_events_total and VM returned it with its labels intact — service.name=offgrid-console, action, outcome, org. Before this VM held zero series because /v1/metrics 404d. CLOSED 2026-08-04 for a named producer end-to-end. Remaining: correlating ACCEPTED sample counts against the producer needs collector self-telemetry, which is tracked on the collector-throughput item rather than duplicated here.'
      ]),
      capability('alerts-recording-rules', 'Alerts and recording rules', 'Create rule groups, evaluate alerts, and inspect firing state.', '/operations/health/metrics/alerts', 'Inspect alerts', 'Rule + firing state are READ from the live vmalert API (engineDeployed confirmed); rule-group authoring is not exposed because vmalert rule definitions are file-provisioned at deploy, not console-writable. No rules are currently configured to prove firing.', [
        'yes', 'The VictoriaMetrics stack supports vmalert recording and alert rules.', 'partial', 'The adapter reads /api/v1/rules + /api/v1/alerts from the live vmalert (verified engineDeployed:true); no rule-write adapter (rules are deploy-owned file config).', 'yes',
         'The Alerts tab lists recording/alerting rules + firing alerts with a summary; no rule-authoring UI.', 'no', 'No rules are currently deployed on the fleet vmalert, so firing/recording state cannot yet be shown against real rules.',
      ]),
      capability('retention-backup-cluster', 'Retention, backup, and cluster operations', 'Know how long metrics are kept; leave backup and cluster topology to the deployment.', '/governance/evidence/retention', 'See retention posture', 'Backup, restore and cluster topology stay deployment-owned DELIBERATELY — they are privileged and destructive, and a browser-triggered restore is not a capability worth having. What was genuinely missing was being able to STATE the retention window; that is closed.', [
        'yes', 'VictoriaMetrics reports its effective flags and supports snapshots and backups natively.',
        'partial', 'Retention is READ BACK from the store\'s own /flags — the deployment reports -retentionPeriod="3", so metrics are kept 3 months. No backup/restore or cluster adapter exists, deliberately.',
        'yes', 'The retention evidence page reports the window per store beside the database sweep, and distinguishes a window the store CONFIRMED from one merely assumed from a built-in default.',
        'yes', 'Measured live 2026-08-05: metrics reported as kept 3 months, CONFIRMED by the store itself, shown on /governance/evidence/retention next to the sweep that covers database records. The retention claim now includes the metric store instead of silently excluding it.',
      ]),
    ],
  },
  {
    ...
    items: [
      ...
      capability('retention-alerts-tenancy', 'Retention, alerts, and tenancy', 'Know how long logs are kept; writing the window stays a deploy flag.', '/governance/evidence/retention', 'See retention posture', 'THIS DEPLOYMENT SETS NO LOG RETENTION FLAG, so logs run on the built-in 7-day default — reported as ASSUMED, never as configured, because nobody chose it and the store will not state it. Setting it is a deploy flag on the single-node service. Per-tenant log retention does not exist upstream on this topology.', [
        'yes', 'VictoriaLogs supports a retention flag and stream selectors.',
        'partial', 'Retention is read back from the store; there is no write path, by design. Per-tenant retention is not available on a single-node deployment.',
        'yes', 'The retention evidence page states the log window and marks it ASSUMED rather than confirmed, so an auditor asking who chose it is told that nobody did.',
        'yes', 'Measured live 2026-08-05 and the reading is the finding: the flag list contains no retention flag at all. The surface says logs are kept 7 days on a built-in default nobody set, and the deployment-wide summary refuses to make a blanket retention claim while ANY store is unproven — one unconfirmed store is enough, because the claim is about all of the data.',
      ]),
    ],
  },
  stale({
    ...
    items: [
      ...
      capability('trace-export-jaeger', 'Trace export to Jaeger', 'Batch accepted traces and export them to Jaeger OTLP.', '/operations/health/traces', 'Open trace explorer', 'DURABLE CORRELATION CLOSED 2026-08-04. Remaining: exporter-FAILURE state per run — a run whose spans never left is still indistinguishable from one that was never traced, and that needs instrumentation inside the OTLP export path, not a caption.', [
        'yes', 'The collector supports OTLP exporters.', 'yes', 'Correlation is now real. The audit span carried action/actor/outcome and NO run key, so a run\'s trace was unfindable — and a trace id DERIVED from the run id cannot work either, because the emitter generates its own (normalizeTraceId is Langfuse\'s arbitrary-string form, not a 128-bit W3C id; a link built on it produced "apprun60a23031", which is not even hex). The audit event already carried runId and org; both are now span tags, and traceLookup SEARCHES by run_id instead of fabricating an id. The static exporter config remains deployment-owned, which is correct.', 'yes', 'Platform Health reads Jaeger traces.', 'partial', 'Proven live 2026-08-04 after restarting the worker: a fresh run\'s id searched by tag returned 2 traces, spans carrying run_id + org alongside action/actor/outcome. Spans emitted before this carry no run key, so older runs will not match — a real limit, stated rather than hidden.',
      ]),
      capability('otlp-metrics', 'OTLP metrics and remote write', 'Receive metrics and export them to VictoriaMetrics.', '/operations/health/metrics', 'Open platform metrics', '', [
        'yes', 'The collector supports OTLP metrics and Prometheus remote write.', 'yes', 'A metrics pipeline now exists in the repo-owned otel-collector-g5.yml, exporting OTLP cross-node to VictoriaMetrics by mDNS NAME — VM runs on S1 under OrbStack while the collector runs on g5 under Colima, so a container-name exporter could never resolve, and a name rather than an IP is what stops a DHCP lease breaking it. The console side is emitCounter/emitGauge on the same OTLP wire as spans, targeting the collector only (Langfuse ingests traces, not metrics, so sending it a metrics envelope would 4xx on every emit).', 'yes', 'Platform Health reads VictoriaMetrics.', 'yes', 'Proven live 2026-08-04: /v1/metrics 200 (traces still 200), collector logs clean, and VictoriaMetrics returned its FIRST application series — offgrid_audit_events_total{action,org,outcome,service.name="offgrid-console"}. The producer is every governed audit event. CLOSED 2026-08-04, and the gap text had the cause wrong: it asked for a producer, but POST /v1/metrics returned 404 while /v1/traces returned 200 — the deployed collector had traces and logs pipelines and NO metrics pipeline, so no producer could ever have populated VictoriaMetrics. Remaining: correlating ACCEPTED vs EXPORTED sample counts needs collector self-telemetry, tracked on the throughput item.',
      ]),
      ...
      capability('self-telemetry', 'Collector throughput, errors, and queues', 'Inspect accepted signals, exporter failures, batches, and queue depth.', '/operations/health/metrics', 'Inspect collector metrics', 'The collector\'s own counters reach the metrics store and are queryable, but nothing ALERTS on them — a collector that silently stops exporting is still only discoverable by someone looking. Rule authoring is tracked on the VictoriaMetrics alerts item, so the remaining work is one rule, not a surface.', [
        'yes', 'The collector emits receiver, processor, and exporter metrics.', 'yes', 'A verified self-telemetry export path now exists: service.telemetry.metrics with a periodic OTLP reader to VictoriaMetrics. GOTCHA worth keeping — the telemetry reader posts to the endpoint EXACTLY as written while the otlp_http pipeline exporter appends /v1/metrics itself, so the same base URL works for one and 400s for the other ("unsupported path requested: /opentelemetry"). The reader needs the full /opentelemetry/v1/metrics.', 'yes', 'Platform Health has collector metric queries and they now return values.', 'partial', 'Proven live 2026-08-04: VictoriaMetrics holds otelcol_receiver_accepted_spans / _metric_points, otelcol_exporter_sent_spans / _metric_points, otelcol_exporter_queue_size / _capacity, otelcol_processor_memory_limiter_accepted_*, plus process CPU/RSS/uptime. Accepted and exported are both present, so the accepted-vs-exported correlation the ingest item wanted is queryable. CLOSED 2026-08-04 for self-metrics reaching the store. Remaining: an ALERT that consumes them — vmalert rule authoring is tracked on the VictoriaMetrics alerts item, so the counters are queryable and not yet alerted on.'
      ]),
    ],
  }),
  {
  ...
  stale({
    ...
    items: [
      ...
      capability('connector-lifecycle', 'Connector and credential lifecycle', 'Create, inspect, update, test, rotate credentials, and delete the binding.', '/data/sources', 'Manage connector', 'Prove lifecycle and secret rotation for the internal fixture without bypassing the SSRF policy.', [
        'yes', 'MySQL supports credentialed connectivity and schema discovery.', 'partial', 'MySQL connector CRUD is ready, but the internal fixture is seeded outside public self-serve validation.', 'yes', 'Data Sources exposes connector and secret management.', 'yes',
         'ROTATION PROVEN LIVE 2026-08-05 on this fixture: the inline password was moved into OpenBao (credential-free endpoint + org-scoped secret_ref) and the connector still connected; the MySQL password was then changed and the STALE vault entry was observed being REFUSED ("Access denied ... using password: YES") before the vault was updated and the connection recovered. The refusal is the evidence — a rotation never seen failing has not been shown to do anything. Restored afterwards and all four connectors on that user re-verified. Seeds still ship inline credentials (G-211). docs/evidence/2026-08-05-credential-rotation.md',
      ]),
  ...
  },
  {
    ...
    items: [
      capability('object-read-write', 'Governed object read and write', 'Read and write objects under a tenant-approved bucket and prefix.', '/data/sources', 'Open data sources', '', [
        ...
        'yes', 'PROVEN LIVE 2026-08-05 on org_suraksha, both halves in one app. Read: apprun_cd297a95 read 2 claim intimations with per-object etag+sha256 provenance, refs con_1b29a2:suraksha-claim-lake/intimations. Write: apprun_f7c36c00 saved assessments/assessment-apprun_f7c36c00.json, PII masked before send, receipt retained. Negatives all refused: a payroll-scoped domain sees only payroll; ../ escapes answer scope-denied; a foreign tenant gets unknown-source. AND the pipeline HARD CEILING gates both — revoking the output domain made the write fail (apprun_eee51b30, no object written), re-approving let it through. That last one was a real hole this proof found and closed: the ceiling gated reads only, so the new sink was a way out of it. docs/evidence/2026-08-05-governed-object-io.md',
      ]),
      capability('bucket-discovery', 'Bucket and object discovery', 'List the buckets and folders a source can see, to scope a data rule to one of them.', '/data/domains', 'Declare a data domain', '', [
        'yes', 'The pinned S3-compatible release exposes bucket and object listing.',
        'yes', 's3-source-discovery.ts lists a source\'s buckets and a bucket\'s folders using THAT connector\'s endpoint and its own vaulted keypair. The connector id is the only caller input — a discovery call that accepted a URL would be a request-forgery primitive wearing an admin route. It returns names and counts, never object CONTENT: the data rule being configured IS the approval that does not exist yet.',
        'yes', 'The data-domain form replaces its free-text resource field with a bucket + folder picker when the source is an object store. An unreachable source says so instead of rendering as a source with no buckets, and objectsAtRoot distinguishes "no folders" from "empty bucket".',
        'yes', 'PROVEN LIVE 2026-08-05 on the deployment: the picker listed the real buckets [claims-intake-lake, media, provit] and that bucket\'s real folders [assessments/, intimations/, surveyor-reports/], with the free-text field absent (screenshot verified). Source onboarding consumes it — this is how an S3 data rule gets its scope now. Four negatives refused: no saved keypair answers missing-credential, a postgres source answers not-object-store, a foreign tenant answers unknown-source, and an unreachable store answers unreachable rather than an empty list. Closing the guess-and-fail-at-run-time loop is the point: a bucket typed from memory used to save cleanly and fail later as "no records".',
      ]),
      capability('versioning-retention-events', 'Versioning, retention, policies, and events', 'Know whether previous versions survive, and how long files are kept.', '/data/lake', 'Open object store', 'Only the STATE is surfaced. Turning versioning on or off, object lock, and bucket event notifications stay deployment-owned: they are privileged and partly irreversible, and the store has already been measured to silently truncate a lifecycle window over 255 days (G-209), so a compliance control built on it would need measuring per operation before it could be believed. Bucket events are unmeasured on this deployment.', [
        'yes', 'The S3-compatible API exposes versioning, retention and notification sub-resources.',
        'partial', 'Retention (lifecycle) is read, written and cleared; versioning STATE is read via getVersioning, which distinguishes "the store does not implement this" from "versioning is off" — a bucket with no history because the store cannot do it is not a bucket someone chose not to version. Measured live 2026-08-05: this store returns supported=true, status=Suspended. Object lock and bucket events have no adapter and are not claimed.',
        'yes', 'The bucket panel states both plainly: how long files are kept, and that previous versions are NOT kept — so overwriting a file here is permanent. An unreadable versioning state says so rather than implying versions exist.',
        'partial', 'The retention half is bound to a real workflow (lake_objects in the retention sweep, proven live — docs/evidence/2026-08-05-lake-retention.md). Versioning is OBSERVED, not managed, and no workflow turns it on. That is deliberate while the store\'s own lifecycle encoding is known broken for long windows.',
      ]),
    ],
  },
  stale({
