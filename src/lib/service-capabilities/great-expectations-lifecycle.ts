...
export interface GxLifecyclePort {
  ...
  updateSuite(
    context: GxTenantContext,
    name: string,
    input: GxSuiteUpdate,
  ): Promise<GxResult<GxSuite>>;
  deleteSuite(
    context: GxTenantContext,
    name: string,
    expectedVersion?: number,
  ): Promise<GxResult<{ deleted: true }>>;
  runValidation(
    context: GxTenantContext,
    input: GxValidationRequest,
  ): Promise<GxResult<GxValidationRun>>;
...
function identifier(value: unknown, label: string, errors: string[]): string {
  ...
  if (!IDENTIFIER.test(normalized))
    errors.push(`${label} must be 1-128 letters, numbers, dots, dashes, or underscores.`);
  return normalized;
}
...
export function parseTenantContext(input: unknown): ParseResult<GxTenantContext> {
  ...
  if (!actor || actor.length > 320)
    errors.push('actor is required and must be at most 320 characters.');
  return errors.length
    ? { ok: false, errors, value: null }
    : { ok: true, errors: [], value: { orgId, actor } };
}
...
function parseExpectations(value: unknown, errors: string[]): GxExpectationSpec[] {
  ...
  return value.map((candidate, index) => {
    ...
    if (!EXPECTATION.test(type))
      errors.push(`expectations[${index}].type must be a Great Expectations expectation name.`);
...
export function parseSuiteUpdate(input: unknown): ParseResult<GxSuiteUpdate> {
  ...
  const expectations =
    raw.expectations === undefined ? undefined : parseExpectations(raw.expectations, errors);
  if (description === undefined && expectations === undefined)
    errors.push('at least one of description or expectations is required.');
  return errors.length
    ? { ok: false, errors, value: null }
...
export function parseProfileRequest(input: unknown): ParseResult<GxProfileRequest> {
  ...
  const sampleLimit = positiveInteger(
    raw.sampleLimit,
...
export function parseValidationRequest(input: unknown): ParseResult<GxValidationRequest> {
  ...
  if (batchRaw.kind === 'inline') {
    const rows = Array.isArray(batchRaw.rows)
      ? batchRaw.rows.map(object).filter((row): row is Record<string, unknown> => row !== null)
      : [];
    if (
      !Array.isArray(batchRaw.rows) ||
      rows.length !== batchRaw.rows.length ||
  ...
  const idempotencyKey =
    raw.idempotencyKey === undefined
      ? undefined
      : identifier(raw.idempotencyKey, 'idempotencyKey', errors);
  return errors.length
    ? { ok: false, errors, value: null }
...
export function parseHistoryQuery(input: unknown): ParseResult<GxHistoryQuery> {
  ...
  const errors: string[] = [];
  const suiteName =
    raw.suiteName === undefined || raw.suiteName === ''
      ? undefined
      : identifier(raw.suiteName, 'suiteName', errors);
  const dataSourceId =
    raw.dataSourceId === undefined || raw.dataSourceId === ''
      ? undefined
      : identifier(raw.dataSourceId, 'dataSourceId', errors);
  const limit = positiveInteger(
    raw.limit,
...
export function unavailableManifest(
  reason: string,
  reachable = false,
  engine: GxEngine = 'unknown',
): GxCapabilityManifest {
  return {
    serviceReachable: reachable,
    engine,
    engineVersion: null,
    operations: Object.fromEntries(GX_OPERATIONS.map((operation) => [operation, false])) as Record<
      GxOperation,
      boolean
    >,
    reason,
...
export function parseCapabilityManifest(input: unknown): GxCapabilityManifest {
  ...
  const engine: GxEngine =
    engineRaw === 'great-expectations'
      ? 'great-expectations'
...
export function operationUnavailable<T>(
  manifest: GxCapabilityManifest,
  operation: GxOperation,
): GxResult<T> {
  const engine =
    manifest.engine === 'great-expectations'
      ? 'Great Expectations'
      : 'the native compatibility engine';
  return {
    ok: false,
    kind: 'unavailable',
    status: 501,
