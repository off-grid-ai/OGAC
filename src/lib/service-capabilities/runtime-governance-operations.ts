  ...
  audit({
    ...
    capabilities: [
      ...
      [
        ...
        '/operations/runs',
        'Manage runs',
        'Cancel and terminate are proven live on durable app-run workflows; retry and replay (reset) are not yet implemented — they re-execute the run and are deferred until the guard tier has headroom.',
        [
          'yes',
          'Temporal supports termination, cancellation, reset/replay, and retry semantics.',
          'partial',
          'cancelWorkflow cancels/terminates real durable app-run workflows (proven live) and the row is reconciled to cancelled; reset/replay and a complete retry matrix are not yet exposed.',
          'partial',
          'The app Runs tab exposes cancel/terminate for in-flight (running/awaiting_human) runs; retry/replay controls are not yet present.',
          'partial',
          'Verified live on the fleet 2026-07-20: paused run apprun_46dcf870 was terminated via POST /api/v1/admin/app-runs/{id}/cancel (200), its Temporal workflow apprun-bhapp_fnol-apprun_46dcf870 killed and the run row reconciled to cancelled with a finished timestamp; the pure eligibility guard returned 409 on a terminal (done) run. Retry/replay were not exercised.',
        ],
      ],
      [
        'worker-readiness',
