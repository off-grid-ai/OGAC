  ...
  },
  {
    ...
    items: [
      ...
      defineCapability(
        ...
        '/insights/cost',
        'Open cost insights',
        'Cost is attributed by model/pipeline/user/project/time-window from the real traffic log (console-native FinOps). Remaining: per-LiteLLM-virtual-key attribution (needs traffic through virtual keys) + reconciling LiteLLM callback cost as a cross-check.',
        [
          'yes',
          'LiteLLM calculates spend and exposes cost data; the console additionally meters the audit/traffic log with per-model pricing (finops.priceFor/costForTokens).',
          'yes',
          'assembleFinOps buckets real per-request token counts × price into cost by model/user/project; LiteLLM /global/spend/keys is also readable per virtual key.',
          'yes',
          'Insights → Cost (Models/Users/Projects/Overview) attributes spend with pipeline + time-window filters (24h/7d/30d/90d/all).',
          'yes',
          'Fleet-proven live: cost-by-model shows real usage (Qwen 2.5 14B — 53 requests, 74,808 tokens; GPT-4o-mini — 4 requests) priced into spend, filterable by pipeline + window; local models correctly report $0.',
        ],
      ),
      defineCapability(
        'response-cache',
