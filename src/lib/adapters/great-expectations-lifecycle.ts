...
function decodeSuite(value: unknown): GxSuite | null {
  ...
  if (!raw || !text(raw.name) || !Number.isInteger(raw.version) || (raw.version as number) < 1)
    return null;
...
function decodeProfile(value: unknown): GxProfileResult | null {
  ...
  if (
    !Number.isInteger(raw.sampledRows) ||
    (raw.sampledRows as number) < 0 ||
  ...
  const columns = raw.columns.map((candidate) => {
    ...
    const distinctCount =
      column?.distinctCount === null ? null : finiteNumber(column?.distinctCount);
    if (
      !column ||
      !text(column.name) ||
    ...
    const min =
      typeof column.min === 'string' || typeof column.min === 'number' ? column.min : null;
    const max =
      typeof column.max === 'string' || typeof column.max === 'number' ? column.max : null;
    return {
      name: text(column.name),
      inferredType: text(column.inferredType),
      rowCount,
      nullCount,
      distinctCount,
      min,
...
function decodeValidation(value: unknown): GxValidationRun | null {
  ...
  if (
    ...
    !text(raw.suiteName) ||
    !text(raw.startedAt) ||
    !text(raw.completedAt)
  )
    return null;
  if (
    typeof raw.success !== 'boolean' ||
    !Number.isInteger(raw.evaluated) ||
    !Number.isInteger(raw.failed)
  )
    return null;
  ...
  const outcomes = raw.outcomes.map((candidate) => {
    ...
    if (
      ...
      !Number.isInteger(outcome.unexpectedCount)
    )
      return null;
    return {
  ...
  const engine =
    raw.engine === 'great-expectations' || raw.engine === 'native-compatibility'
      ? raw.engine
      : 'unknown';
  return {
    id: text(raw.id),
...
export function createGreatExpectationsLifecycleAdapter(
  config: AdapterConfig = {},
): GxLifecyclePort {
  const configuredUrl = () =>
    (
      config.baseUrl ??
      process.env.OFFGRID_GX_LIFECYCLE_URL ??
      process.env.OFFGRID_DATAQUALITY_URL ??
      DEFAULT_URL
    ).replace(/\/$/, '');
  ...
  async function capabilities(context: GxTenantContext): Promise<GxCapabilityManifest> {
    ...
    if (!headers) {
      return unavailableManifest(
        'Great Expectations lifecycle credentials or tenant context are not configured.',
      );
    }
    try {
      ...
      if (!response.ok) {
        return unavailableManifest(
          `Great Expectations lifecycle manifest returned HTTP ${response.status}.`,
          response.status < 500,
        );
      }
      ...
      if (!manifest.serviceReachable)
        return {
          ...manifest,
          reason: manifest.reason ?? 'Great Expectations lifecycle service is not ready.',
        };
      return manifest;
    } catch (error) {
      return unavailableManifest(
        `Great Expectations lifecycle service is unreachable: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }
  ...
  async function request<T>(context: GxTenantContext, spec: RequestSpec<T>): Promise<GxResult<T>> {
    ...
    if (!parsedContext.ok || !parsedContext.value) {
      return failure(
        unavailableManifest('Tenant context is invalid.'),
        'invalid',
        parsedContext.errors.join('; '),
        400,
      );
    }
    ...
    if (!headers)
      return failure(
        manifest,
        'upstream',
        'Great Expectations lifecycle credentials are unavailable.',
        502,
      );
    ...
    try {
      ...
      const body =
        response.status === 204 ? { deleted: true } : await response.json().catch(() => null);
      if (!response.ok) {
        if (response.status === 400 || response.status === 422)
          return failure(
            manifest,
            'invalid',
            responseMessage(body, 'Great Expectations rejected the request.'),
            400,
          );
        if (response.status === 404)
          return failure(
            manifest,
            'not-found',
            responseMessage(body, 'Great Expectations resource was not found.'),
            404,
          );
        if (response.status === 409)
          return failure(
            manifest,
            'conflict',
            responseMessage(body, 'Great Expectations resource version conflicts.'),
            409,
          );
        return failure(
          manifest,
          'upstream',
          responseMessage(body, `Great Expectations returned HTTP ${response.status}.`),
          502,
        );
      }
      ...
      return failure(
        manifest,
        'upstream',
        `Great Expectations request failed: ${error instanceof Error ? error.message : String(error)}`,
        502,
      );
    }
  }
  ...
  return {
    ...
    getSuite: (context, name) =>
      request(context, {
        operation: 'suite.read',
        method: 'GET',
        path: `/v1/suites/${encodeURIComponent(name)}`,
        decode: decodeSuite,
      }),
    createSuite: (context, input) =>
      request(context, {
        operation: 'suite.create',
        method: 'POST',
        path: '/v1/suites',
        body: input,
        decode: decodeSuite,
      }),
    updateSuite: (context, name, input) =>
      request(context, {
        operation: 'suite.update',
        method: 'PATCH',
        path: `/v1/suites/${encodeURIComponent(name)}`,
        body: input,
        decode: decodeSuite,
      }),
    deleteSuite: (context, name, expectedVersion) =>
      request(context, {
        operation: 'suite.delete',
        method: 'DELETE',
    ...
    history: (context, query) => {
      ...
      return request(context, {
        ...
        decode: (body) => {
          ...
          return {
            runs: runs as GxValidationRun[],
            nextCursor: raw.nextCursor === null ? null : text(raw.nextCursor) || null,
          } satisfies GxHistoryPage;
        },
      });
    },
  };
