            ...
            <div className="space-y-2">
              {a.drift.flagged ? (
                <div className="flex items-center gap-2 rounded-md border border-amber-500/30 bg-amber-500/10 px-3 py-2 text-sm text-amber-700">
                  ...
                  Drift detected — blocked/redacted rate {(a.drift.recent * 100).toFixed(0)}% recent
                  vs {(a.drift.baseline * 100).toFixed(0)}% baseline.
                </div>
              ) : null}
              {a.perf.flagged ? (
                <div className="flex items-center gap-2 rounded-md border border-amber-500/30 bg-amber-500/10 px-3 py-2 text-sm text-amber-700">
                  <AlertTriangle className="size-4" />
                  Performance degradation — p95 latency {a.perf.recent} ms recent vs{' '}
                  {a.perf.baseline} ms baseline.
                </div>
              ) : null}
            </div>
          ) : null}
          ...
          <Card className="shadow-sm">
            <CardHeader>
              ...
              <p className="mt-1 text-xs text-muted-foreground">
                ...
                scenes and the console renders the result — no embedded UI, no blank iframe. Power
                users can open Superset directly to author new charts.
              </p>
            </CardHeader>
            <CardContent>
