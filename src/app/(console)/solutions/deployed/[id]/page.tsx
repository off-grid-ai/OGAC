...
export default async function DeploymentDetailPage({
  ...
  const [blueprint, app] = await Promise.all([
    getSolutionBlueprint(deployment.blueprintId, orgId),
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        <Link href={`/solutions/library/${blueprint.id}`} className="rounded-lg border bg-card p-5">
          ...
          <p className="mt-2 font-medium">Blueprint v{blueprint.proof.version}</p>
        </Link>
        <Link href={`/solutions/apps/${app.id}/runs`} className="rounded-lg border bg-card p-5">
          <p className="text-xs text-muted-foreground">Execution evidence</p>
          <p className="mt-2 font-medium">Open App runs →</p>
        </Link>
        <Link href={`/solutions/apps/${app.id}/reports`} className="rounded-lg border bg-card p-5">
          <p className="text-xs text-muted-foreground">Measured evidence</p>
          <p className="mt-2 font-medium">Open App reports →</p>
        </Link>
      </div>
