    ...
    <PageFrame>
      {
        <div className="w-full space-y-5">
          ...
          <StatRail at="sm" cols={4}>
            ...
            <Meta
              label="Started"
              value={row.startedAt ? new Date(row.startedAt).toLocaleString() : '—'}
            />
            <Meta label="Duration" value={describeDuration(durationMs)} />
            <Meta
              label={isModelPipeline ? 'Model' : 'Pipeline'}
              value={pipelineLabel}
              mono={!isModelPipeline}
            />
          ...
          {agent ? (
            <div className="space-y-4">
              ...
              {agent.provenance ? (
                <p
                  ...
                  title={agent.provenance.signature}
                >
            ...
            <Section title="Chat run">
              <p className="text-sm text-muted-foreground">
                A governed chat turn — recorded with its guardrail outcome and correlated by run id
                for the audit trail. Open the conversation to see the full exchange.
              </p>
            </Section>
          ) : null}
        </div>
...
function Meta({ label, value, mono }: Readonly<{ label: string; value: string; mono?: boolean }>) {
  return (
    <div className="rounded-md border border-border p-3">
      ...
      <div
        className={`mt-0.5 truncate text-sm text-foreground ${mono ? 'font-mono text-xs' : ''}`}
        title={value}
      >
        {value}
      </div>
