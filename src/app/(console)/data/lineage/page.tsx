      ...
      jobsBody = data.jobs.map((j) => (
        ...
        >
          ...
          <div className="space-y-1">
            {j.outputs.length ? (
              j.outputs.map((o) => (
                <div key={o} className="flex items-center gap-1.5 rounded-md bg-muted/50 px-2 py-1">
        ...
        <div className="space-y-6">
          <div className="flex items-start justify-between gap-4">
            <p className="max-w-2xl text-sm text-muted-foreground">
              Data lineage for every agent run — which sources fed which answer, end to end. Each
              run records a lineage event; the graph below is read back from the lineage store, with
              a fallback reconstruction from recorded source→answer edges.
            </p>
            <Badge variant="secondary" className="bg-primary/10 text-primary">
              Data lineage
            </Badge>
