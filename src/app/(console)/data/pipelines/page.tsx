...
export default async function PipelinesPage() {
  ...
  const allJobs = [...jobsByConn.values()].flat().sort((a, b) => (b.createdAt ?? 0) - (a.createdAt ?? 0));
      ...
      <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
        <div>
          ...
          <p className="mt-1 max-w-2xl text-xs text-muted-foreground">
            Pipelines move data from your sources into the warehouse. Trigger a sync on demand and
            watch each run land — this is how fresh data gets in.
          </p>
        </div>
      </div>
                ...
                <Card key={c.connectionId} className="shadow-sm">
                  <CardHeader className="space-y-0 pb-2">
                    <div className="flex items-start justify-between gap-2">
                      ...
                      <Badge className={CONN_TONE[c.status] ?? 'bg-muted text-muted-foreground'}>
                        {c.status}
                      </Badge>
                    </div>
