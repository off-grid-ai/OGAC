      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
        <Card className="shadow-sm">
          ...
          <CardContent className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            ...
            <Field label="Last sync">{c.lastSync ? c.lastSync.slice(0, 19).replace('T', ' ') : 'never'}</Field>
              ...
              boundDomains.map((d) => (
                ...
                >
                  <div className="flex flex-col">
                    ...
                    {d.aliases.length > 0 ? (
                      <span className="text-xs text-muted-foreground">{d.aliases.join(', ')}</span>
                    ) : null}
                  </div>
                  <div className="flex items-center gap-1 text-xs">
                    <ArrowRight className="size-3 shrink-0 text-muted-foreground" />
                    <code className="truncate font-mono text-muted-foreground">{d.resource}</code>
                  </div>
                </div>
              ))
            )}
