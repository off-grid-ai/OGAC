...
export async function DataGovernanceContent({
  ...
  const evaluated = assets.map((a) => {
    ...
    const retention = evaluateRetention(
      {
        retainDays: rp?.retainDays ?? 0,
        action: rp?.action,
        legalHold: rp?.legalHold,
        anchorAt: a.lastRefreshAt ?? a.createdAt,
      },
      now,
    );
  ...
  const freshnessSummary = summarizeFreshness(
    evaluated.map((e) => e.freshness) as FreshnessResult[],
  );
    ...
    <PageFrame embedded={embedded}>
      {
        <div className="w-full space-y-6">
          ...
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">
            ...
            <StatCard
              label="Stale"
              value={String(freshnessSummary.stale)}
              tone={freshnessSummary.stale ? 'warn' : undefined}
            />
            <StatCard
              label="Broken sync"
              value={String(freshnessSummary.broken)}
              tone={freshnessSummary.broken ? 'bad' : undefined}
            />
            <StatCard
              label="Due for disposal"
              value={String(dueForDisposal.length)}
              tone={dueForDisposal.length ? 'warn' : undefined}
            />
            ...
            <CardContent>
              {alerts.length === 0 ? (
                <p className="py-4 text-center text-xs text-muted-foreground">
                  No stale or broken datasets. Alerts appear here once a dataset misses its
                  freshness SLA or a sync fails — set an SLA on a dataset in the catalog to arm it.
                </p>
              ) : (
                    ...
                    <TableBody>
                      {alerts.map((e) => (
                        <TableRow key={e.asset.id}>
                          ...
                          <TableCell>
                            <Badge className={FRESH_TONE[e.freshness.state]}>
                              {e.freshness.state}
                            </Badge>
                          </TableCell>
          ...
          <Card className="shadow-sm">
            ...
            <CardContent>
              {dueForDisposal.length === 0 ? (
                <p className="py-4 text-center text-xs text-muted-foreground">
                  Nothing past its retention window. Set a retention policy on a dataset in the
                  catalog to track disposal here.
                </p>
              ) : (
                <div className="overflow-x-auto">
                  <Table>
          ...
          <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
            ...
            <Card className="shadow-sm lg:col-span-2">
              ...
              <CardContent>
                {requests.length === 0 ? (
                  <p className="py-4 text-center text-xs text-muted-foreground">
                    No erasure requests recorded yet. Submit a subject to run a
                    right-to-be-forgotten and record it here — the request captures what was erased
                    in the console plane and what waits on the warehouse data engine.
                  </p>
                ) : (
                  <div className="overflow-x-auto">
                    <Table>
                      ...
                      <TableBody>
                        {requests.map((r) => (
                          <TableRow key={r.id}>
                            ...
                            <TableCell>
                              <Badge className={STATUS_TONE[r.status] ?? STATUS_TONE.recorded}>
                                {r.status}
                              </Badge>
                            </TableCell>
