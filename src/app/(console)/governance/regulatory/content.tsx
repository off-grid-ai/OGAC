...
export type RegulatoryPageProps = Readonly<{
                    ...
                    {f.controlIds.map((id) => {
                      ...
                      return (
                        <Badge
                          key={id}
                          variant="secondary"
                          className={ctrl ? STATUS[ctrl.status] : ''}
                        >
                          {ctrl?.name ?? id}
                        </Badge>
                      );
            ...
            <CardHeader className="space-y-3">
              <div>
                <CardTitle className="text-sm">
                  DPO activity — data processing over a time range
                </CardTitle>
                <p className="mt-1 text-xs text-muted-foreground">
                  Aggregated live from the real audit ledger: who did what, what was blocked or
                  denied, and what it cost — plus provenance signing coverage. Export the DPIA pack
                  for the regulator.
                </p>
              </div>
          ...
          <Card className="shadow-sm">
            <CardHeader className="flex flex-row items-center justify-between space-y-0">
              <div>
                ...
                <p className="mt-1 text-xs text-muted-foreground">
                  The org/regulatory wrapper — policies, committees, and processes tracked as
                  attestable records (Phase E).
                </p>
              </div>
              <AddGovernanceButton />
            </CardHeader>
                    ...
                    <TableRow key={g.id}>
                      ...
                      <TableCell>
                        <div className="flex items-center justify-end gap-1">
                          ...
                          />
                          <DeleteRowButton
                            url={`/api/v1/admin/governance/${g.id}`}
                            label={g.title}
                          />
                        </div>
                      </TableCell>
