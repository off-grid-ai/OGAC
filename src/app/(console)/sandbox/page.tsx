      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Code-execution isolation for agent-authored code — the active backend, its reachability,
            and recent runs. On-prem, gated by the agent-code-exec flag.
          </p>
        </div>
      </div>
        ...
        <CardHeader className="flex flex-row items-center justify-between space-y-0">
          ...
          <div className="flex flex-wrap gap-1">
            {FILTERS.map((f) => (
              <Link
                key={f.id}
                href={f.id === 'all' ? '/sandbox' : `/sandbox?status=${f.id}`}
                scroll={false}
              >
                <Badge
                  variant="secondary"
