          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                Apply a prebuilt evaluator (bias, toxicity, hallucination, PII, and more), build a
                golden set, and run it — scored offline against the gateway with per-metric results.{' '}
              ...
              {view.suites.length === 0 ? (
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  {view.suites.map((s) => (
                    <Link
                      key={s.engine}
                      href={`/build/evals?suite=${encodeURIComponent(s.engine)}`}
                    >
                      <Badge
          ...
          <Card className="shadow-sm">
            <CardHeader className="pb-3">
              ...
              <p className="text-xs text-muted-foreground">
                Pipelines with their own golden set — evals attached on the pipeline&apos;s Quality
                tab run in that pipeline&apos;s context. Org-wide library cases (no pipeline)
                aren&apos;t counted here.
              </p>
            </CardHeader>
            <CardContent>
              {boundPipelines.length === 0 ? (
                <p className="text-xs text-muted-foreground">
                  No pipeline has a golden set yet — attach cases to a pipeline on its Quality tab
                  to gate its quality per-pipeline.
                </p>
              ) : (
                <div className="flex flex-wrap gap-2">
