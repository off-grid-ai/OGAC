...
export async function POST(req: NextRequest, { params }: { params: Promise<{ name: string }> }) {
  ...
  if (!parsed || !VALID_ACTIONS.has(parsed.action)) {
    return NextResponse.json(
      { error: 'action must be one of model|restart|enable|disable' },
      { status: 400 },
    );
  }
  ...
  if (!raw)
    return NextResponse.json({ error: `node "${name}" not found in the pool` }, { status: 404 });
  ...
  if (!decision.ok) {
    ...
    return NextResponse.json(
      { error: decision.reason, notActionable: Boolean(decision.blocked) },
      { status },
    );
  }
