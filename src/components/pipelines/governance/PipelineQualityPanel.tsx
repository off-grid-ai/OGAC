...
export function PipelineQualityPanel({
  ...
  /**
   * The most recent run of each check, read on the server. Without this the tab could never answer
   * "is this app OK right now?" — the result display existed but was filled only by clicking Run in
   * that session, so opening the tab always showed nothing.
   */
  ...
  // THE ANSWER THE TAB EXISTS FOR. A check that has never run is deliberately not counted as passing:
  // "we never checked" and "we checked and it was fine" are different answers and only one means OK.
  const overall = useMemo(
    () => overallVerdict(evals.map((d) => ({ id: d.id, threshold: d.threshold, direction: d.direction })), lastRuns ?? {}),
    [evals, lastRuns],
  );
      ...
      <div className="grid gap-6 xl:grid-cols-2">
        ...
        <Card className="shadow-sm">
          ...
          <CardContent className="space-y-2">
            {/* IS THIS APP OK RIGHT NOW? The question the tab exists to answer, which it could not. */}
            <div
