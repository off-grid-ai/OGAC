...
- **Merge gate (every merge, non-negotiable):** SOLID + pure logic isolated (unit-testable, zero-IO) separated from I/O; **DRY (no duplicated logic)**; thin handlers; REAL tests exercising real behavior (mocks sparingly); **coverage ≥80% on all of branches/statements/lines/functions/conditions (`npm run coverage:check`, enforced by the pre-push hook)**; typecheck clean; tests pass; a clean local production build; verify UI by screenshot (vision) and integration by the harness. Nothing is "done" until VERIFIED live, not merely merged.
