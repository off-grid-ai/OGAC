...
__export(index_exports, {
  ...
  clusterModels: () => models_exports,
  configureRuntime: () => configureRuntime,
  createClusterGateway: () => createClusterGateway,
  dataDir: () => dataDir,
  ...
  keycloakConfigFromEnv: () => keycloakConfigFromEnv,
  langfuseSink: () => langfuseSink,
  limiterConfig: () => limiterConfig,
  modelsDir: () => modelsDir,
  onHostQuit: () => onHostQuit,
  openSearchSink: () => openSearchSink,
  resourceDirs: () => resourceDirs,
  resourceFile: () => resourceFile,
  runPost: () => runPost,
  runPre: () => runPre,
  sinksFromEnv: () => sinksFromEnv,
...
function resourceFile(name) {
  for (const d of resourceDirs()) {
    const p = import_path.default.join(d, name);
    try {
...
function resourceDirs() {
  ...
  return [import_path.default.join(process.cwd(), "resources")];
}
...
function jwkToPublicKey(jwk) {
  if (jwk.kty === "RSA" && jwk.n && jwk.e) {
    return import_crypto.default.createPublicKey({ key: { kty: "RSA", n: jwk.n, e: jwk.e }, format: "jwk" });
  }
  if (jwk.kty === "EC" && jwk.x && jwk.y && jwk.crv) {
    return import_crypto.default.createPublicKey({ key: { kty: "EC", x: jwk.x, y: jwk.y, crv: jwk.crv }, format: "jwk" });
  }
...
var KeycloakValidator = class {
  ...
  async verify(token) {
    ...
    const ok = alg.startsWith("PS") ? import_crypto.default.verify(nodeAlg, sigInput, { key, padding: import_crypto.default.constants.RSA_PKCS1_PSS_PADDING }, sig) : import_crypto.default.verify(nodeAlg, sigInput, key, sig);
    if (!ok) throw new Error("JWT signature invalid");
    return payload;
  }
};
var instances = /* @__PURE__ */ new Map();
function getValidator(cfg2) {
  const k = `${cfg2.url}|${cfg2.realm}|${cfg2.clientId ?? ""}`;
  if (!instances.has(k)) instances.set(k, new KeycloakValidator(cfg2));
...
function createClusterGateway(opts = {}) {
  ...
  const cfg2 = healthConfig(opts.health);
  const traffic = new TrafficStore(sinks);
  const health = new HealthMonitor(traffic, cfg2);
  ...
  const handleRequest = (req, res, url, wantsHtml) => {
    ...
    req.on("end", async () => {
      ...
      const up = import_http.default.request(
        {
          host: target.host,
          port: target.port,
          method: req.method,
...
var TokenStore = class {
  ...
  list() {
    return [...this.byHash.entries()].map(([fingerprint, entry]) => ({ fingerprint, ...entry })).sort((a, b) => b.lastSeen - a.lastSeen);
  }
  get size() {
...
// src/policy/keycloak-auth.ts
function keycloakAuth(opts = {}) {
  const cfg2 = opts.config ?? keycloakConfigFromEnv();
  ...
  return {
    ...
    async pre(ctx) {
      ...
      let claims;
      try {
        claims = await getValidator(cfg2).verify(token);
      } catch (e) {
      ...
      const allRoles = [.../* @__PURE__ */ new Set([...realmRoles, ...resourceRoles])];
