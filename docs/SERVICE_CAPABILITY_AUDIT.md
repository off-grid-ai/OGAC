...
| **AI Gateway / aggregator** | 🟡 PARTIAL | Chat/vision/TTS/STT/image/models/traffic used. Node model/pool config is editable via the **Fleet SSOT** (Control tab → `PATCH /fleet/:name`, pushed to nodes over SSH). Runtime tuning (routing-refresh / health thresholds / upstream timeouts) is now surfaced **read-only** in the **Tuning tab** via the aggregator's new `GET /config` — honestly read-only because those knobs are env-set in the S1 launchd plist (restart to change). **No response cache and no per-request fallback chain** (router has neither; resilience = multi-node pool + hardcoded fallback pool) — surfaced as honest capability flags, not fake controls. The generic per-node control panel (`/nodes/:name` swap/restart) still surfaces as `501 notActionable` (deliberate honesty gate; the Fleet SSOT is the live path). (Rate-limit is Caddy's job — see below — NOT a gap here.) |
| **Keycloak** | 🟢 DEEP | User/role/client lifecycle + client-secret rotation (`keycloak-admin.ts`), **plus realm-level ops** (`keycloak-realm.ts` pure shaping): active sessions (list per-user + revoke one / log-out-all), per-user MFA (OTP status, require/cancel Configure-OTP, remove credential) + realm required-action policy view, IdP federation (list + add OIDC + delete), and token/session lifetimes (read + merge-safe edit of the realm rep). Access page tabs: Sessions / MFA / Federation / Realm. Remaining Keycloak-admin-console-only (deep tails): SAML IdP create + advanced IdP mappers, realm-wide authentication-flow editing. |
