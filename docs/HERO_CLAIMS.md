...
**37 claims: 22 ✅ VERIFIED · 11 🔶 WIRED · 3 🔴 GAP · 1 ⬜ OTHER REPO.**

The three remaining gaps, i.e. everything we say out loud and cannot yet prove:
...
| **B4.10** | "CHEAPER" | Rollups and per-model cost ARE live ($0.2367 / 118k tokens). The defect is ATTRIBUTION: `byKey` is 0 for every key against 121 requests and `bySubject` is empty, so budgets can never be consumed. Re-measured 2026-07-30 |
...
grounding) promoted 🔴 → ✅; and **B2.3** (quality-gating) promoted 🔴 → ✅.

Note what the stale rows still bought us: each was disproved in minutes, and proving B2.3 and B2.4
properly surfaced FOUR real defects the rows had not mentioned — a green gate that could not name its own
checks, a 400 misreported as an outage, and (on B3.1) unjoinable pseudonyms plus corrupted figures. The
value was never in the row's diagnosis; it was in exercising the claim.

**A process finding that outranks any single row: FOUR of these gap rows were STALE** — B4.7, B2.4 and
B2.3 today, after two more on 2026-07-29. The defect they described was already fixed, and each cost minutes to
...
| B2.3 | **Quality-gated** | Data quality expectations | ✅ VERIFIED | **2026-07-30 — the engine is real and the G-F4 row was stale (fourth this week).** `great-expectations` is live at :8944 and DISCRIMINATES: a clean batch → `success:true, 3/3 passed`; a dirty batch (null `claim_no`, `amount:-5`, `status:"teleported"`) → `success:false, 0/3`, each failure naming its rule and column. The row's "0 expectations" is what an EMPTY expectation list returns, which the engine correctly refuses. Two evidence defects found while proving it, both fixed and verified live: a GREEN gate could not say what it verified (passing rules were synthesized as `passed_expectation_1/2/3`; now named by elimination — `expect_column_values_to_not_be_null [claim_no]` etc.), and a 400 was reported as `engineReachable:false` "engine unreachable" (now `rejectedVerdict`: reachable TRUE, "the engine rejected the request"). |
...
| B4.10 | **Cost / FinOps** | Insights → Cost | 🔴 GAP | **PA-6, re-measured live 2026-07-30 — the row was WRONG about what is missing.** Rollups ARE built and carry real numbers: `GET /api/v1/admin/finops` → `totals {requests:121, tokens:118355, costUsd:0.2367, localShare:1}` plus a real `byModel` breakdown (Qwen 2.5 14B $0.1496 / 74,808 tok; Qwen3-VL 8B $0.076; GPT-4o-mini $0.011). And cost is not $0 — `audit_events_v2` holds 18 costed events in 3 days. **The actual defect is ATTRIBUTION: `byKey` reports `requests:0, costUsd:0` for every key while totals show 121 requests, and `bySubject` is `[]`.** So the keys carry budgets (`budgetUsd:120`) that no usage can ever consume — which is why a "budget enforced" demo is vacuous, not because the models are free. **To close:** attribute each model call to its key/subject at spend-record time, then re-check that `byKey` sums to `totals`. Second, open question: `localShare:1` yet non-zero cost — decide what a dollar means for on-prem inference (amortised hardware/energy) and state the basis on screen. **ROOT CAUSE CONFIRMED live 2026-07-30 (schema, not rollup logic):** `assembleFinOps`/`keySpend` are correct — they group on `event.keyId` — but `audit_events_v2` HAS NO KEY COLUMN (`id, ts, actor_type, actor_id, actor_label, org, project, action, resource, model, prompt_tokens, completion_tokens, total_tokens, cost_usd, outcome, run_id, ip`), so `keyId` is always absent and `byKey` can only ever be zero. Worse: every costed event carries `actor_id = service@offgrid.local`, a single service identity, so even per-PERSON attribution is impossible today — the spend is real but anonymous. **The slice:** add key/subject to the spend record, stamp the REAL actor rather than the service account at write time, then assert `byKey` sums to `totals` and a budget can actually be consumed. |
