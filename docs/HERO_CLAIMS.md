...
**37 claims: 22 ✅ VERIFIED · 12 🔶 WIRED · 2 🔴 GAP · 1 ⬜ OTHER REPO.**

The two remaining gaps, i.e. everything we say out loud and cannot yet prove:
...
| B4.10 | **Cost / FinOps** | Insights → Cost | 🔶 WIRED | **Per-subject cost attribution is LIVE and verified 2026-07-30; budget ENFORCEMENT is not.** The defect was one line — `analytics.ts` discarded the gateway's `caller` (the virtual-key alias / end-user id, `litellm-log-shape.ts:109`) and hardcoded `keyId: null`, forcing every bucket to zero. Now carried through (`AuditEvent.caller`, `keyRowFor()`). Live `GET /api/v1/admin/finops`: `bySubject` reports real spend per subject — `org_bharat:fraud-screening` $0.0388, `loan-underwriting` $0.0371, `kyc-rekyc` $0.0241, `cross-sell` $0.0216, `demo-bank@getoffgridai.co` $0.0205 — and **the closing test passes: bySubject requests sum to `totals.requests` (121 = 121), so no traffic silently vanishes from attribution.** **What remains:** `byKey` is still empty, and the labels show why — the gateway bills against its OWN virtual-key aliases (`org_bharat:fraud-screening`), which are not our console `api_keys` rows (`Priya Sharma (Ops Lead)` / `priya.sharma@…`). So budgets exist on objects the traffic never names. Reconcile the two key identities (create console rows matching the gateway aliases, or read LiteLLM's own key list) and the budget bar becomes real. Until then "budget enforced" stays unproven — but "CHEAPER" now has attributable per-subject spend behind it. |
