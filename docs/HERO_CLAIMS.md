...
| **B4.10** | "CHEAPER" | Rollups, per-model cost and per-person attribution are ALL live. The defect is one line: `analytics.ts:62` hardcodes `keyId: null`, which forces `byKey` to 0 and `bySubject` to `[]`, so budgets can never be consumed. Traced 2026-07-30 |
...
| B4.10 | **Cost / FinOps** | Insights → Cost | 🔴 GAP | **PA-6 — traced to one line, 2026-07-30. Two of my own earlier diagnoses here were WRONG; this is the measured chain.** Rollups are real and correct: `GET /api/v1/admin/finops` → `totals {requests:121, tokens:118355, costUsd:0.2367}` with a real `byModel` split, and `assembleFinOps`/`keySpend` group properly on `event.keyId`. **The defect is that `keyId` is hardcoded `null` at the reader** — `src/lib/analytics.ts:62`, in `gatewayEvents()`, which maps the gateway traffic index into `AuditEvent` and puts the caller identity into `deviceId` (`s.caller ?? s.gateway`) while setting `keyId: null`. Everything downstream is then forced to zero: `byKey` reports 0 for every key and `bySubject` is `[]` because it filters `events.filter(e => e.keyId)`. So budgets (`budgetUsd:120`) can never be consumed. **NOT a schema migration** (my earlier note claimed that — wrong) and **spend is NOT anonymous** (also wrong: `audit_events_v2` attributes per person — `demo-bank@getoffgridai.co` $0.0104, `demo-insurer@…` $0.0013, `trigger:webhook` $0.0053; `service@offgrid.local` is the correct label for token-authenticated automation, which is what my own probe traffic was). **The slice:** carry the gateway doc's caller/key identity through `gatewayEvents` instead of discarding it, have `bySubject` fall back to the actor when no virtual key is known, then assert `byKey` + `bySubject` sum to `totals` and that a budget can be consumed. Confirm first what identity field the gateway actually writes (OpenSearch direct read needs auth — use the console's own path). |
