...
- **Clear `.next` before a production build when you've switched branches/commits — a stale webpack cache throws a SPURIOUS `TypeError: Cannot read properties of undefined (reading 'length')` at `WasmHash._updateWithBuffer`.** The stack is entirely inside `node_modules/next/dist/compiled/webpack/bundle5.js` — it looks like your code broke the build, but it's a poisoned incremental cache (worse under node ≥ 24's WASM hasher). Cost me ~5 build cycles + a false "the merge broke the build" attribution. Fix: `rm -rf .next && npm run build`. Only trust a build gate run on a clean cache. The fleet builds on node 22; local is node 26 — build clean on both before trusting green/red.
