...
**Method:** `grep` across `*.md`, `*.ts`, `*.tsx`, `*.mjs`, `*.js`, `*.json`, `*.sh`, `*.yml`, `*.yaml`, `*.sql`, `*.env*`, excluding `node_modules/` and `.next/`, for: LAN IPs (`192.168.*`), internal hostnames (`*.local`), internal service subdomains (`*.getoffgridai.co`), gateway keys (`oglb_`), other key/token shapes (`sk-`, `AKIA`, `Bearer` literals), and credential literals (`changeme`, `offgrid-dev*`, `*_SECRET`, `*_TOKEN`).
...
| `changeme` (real console/Keycloak password) | 17 |
...
- **`changeme`**: real console / Keycloak user password. Rotate the account passwords.
...
- `scripts/metrics-dashboard.mjs:102`: `mac@`, `mohammed.ali@`, `diksha.sharma@`, `ali@example.com`.
- `scripts/shoot-*.mjs`, `scripts/shoot-walkthrough.mjs`, `w1verify.mjs`: `mac@example.com` / `mohammed.ali@example.com` as login users.
