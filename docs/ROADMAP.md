...
| **7** | **Carry the caller identity into FinOps (B4.10).** Traced to `src/lib/analytics.ts:62`: `gatewayEvents()` hardcodes `keyId: null` and puts the caller into `deviceId`, so `byKey` is 0 for every key and `bySubject` is `[]` — budgets can never be consumed. Rollups, per-model cost and per-person attribution all already work. Fix the reader (not the schema — an earlier note claiming a migration was wrong), have `bySubject` fall back to the actor, then assert the buckets sum to `totals`. | B4.10 🔴 |
