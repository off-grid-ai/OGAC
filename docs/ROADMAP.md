...
| # | Item | Claim | Why now |
|---|---|---|---|
| **1** | **Case-scope the auto-inserted data read.** The end-to-end run is proven (compile → save → 11 real cases → run → `awaiting_human`, run `apprun_0c63589e`), but the read `agent-data-dependency.ts` inserts has no case filter, so the agent gets 20 unrelated quota rows and correctly refuses to decide. Compile time cannot know the resource's columns; run time can — `executeConnectorStep` already holds the case record. Also switch the compiled prompt's currency to ₹. | B3.1 🔴 | The sentence on screen in Beat 3 and the founder's north star. B3.2–B3.4 (inherits data, inherits rules, human review) are now ✅, so the expensive half is already done — this is the single biggest credibility win left, and the cheapest to reach. |
