...
| # | Item | Claim | Why now |
|---|---|---|---|
| **1** | **Value-stable pseudonyms (GAP M1) — masking must not destroy entity identity.** A run masks in several places independently and the placeholder counter is per-scan, so one person becomes `[REDACTED_PERSON_23]` in the case and `[REDACTED_PERSON_12][REDACTED_PERSON_13]` in the source rows — and the agent correctly concludes they are different people (`apprun_a60fcc2f`). Derive the token from the VALUE so it is stable across every scan, channel and step; needs the analyzer spans, since the current token is minted after the original value is gone. Unblocks B3.1 and is a stronger claim than the page currently makes. | B3.1 🔶 |
