...
| **7** | **Reconcile gateway virtual keys with console key rows (B4.10).** Per-subject spend is now live and verified (`bySubject` sums to `totals`, 121=121). `byKey` stays empty because the gateway bills against its own aliases (`org_bharat:fraud-screening`) while our budgets sit on unrelated `api_keys` rows. Reconcile the two, then a budget bar is real. | B4.10 🔶 |
