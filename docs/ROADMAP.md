...
| # | Item | Claim | Why now |
|---|---|---|---|
| **1** | ~~Value-stable pseudonyms (GAP M1)~~ **DONE 2026-07-30.** Token derived from the value, salted per org, wired at the one `maskOrBlock` seam so all four call sites inherit it. Verified live: `apprun_76864dd2` reaches the correct quota decision on masked data and pauses for the manager. Also fixed the columnar row output that was corrupting the figures the agent needed. **B3.1 is now ✅ VERIFIED.** | B3.1 ✅ |
| **2** | ~~Vocabulary check + sweep~~ **check DONE 2026-07-30** — `scripts/check-hero-vocabulary.mjs` + baseline (28 terms, 300 flagged files of 777) is a live ratchet, adversarially verified to fail on a rise. The 300 standing leaks remain to clear, incl. the `/insights/ai/langfuse-datasets/` route (G-UX4). | P3 🔶 |
| **3** | ~~Real provenance on a real run~~ **DONE 2026-07-30 — the entry was STALE.** A freshly executed run (`run_8cb8f882`) verifies Ed25519 against the active key through the real verify route, and tamper-evidence is proven, not assumed: mutating the stored answer flips the verdict to `tampered` and restoring it returns `verified`. | B4.7 ✅ |
| **4** | ~~Entailment-grade grounding~~ **DONE 2026-07-30 — the entry was STALE.** The model adapter was already active on the box; verified through the real endpoint with discriminating cases (paraphrase → supported score 1; contradiction → score 0). | B2.4 ✅ |
...
| **6** | ~~Data quality off the stub~~ **DONE 2026-07-30 — the entry was STALE.** `great-expectations` is live and discriminates (clean → 3/3 pass; dirty → 0/3 with each rule named). Fixed two evidence defects found while proving it: a green gate now names the rules it verified, and a 400 reads as a refusal rather than an outage. | B2.3 ✅ |
| **7** | **Attribute spend to a key/subject (B4.10).** Re-measured 2026-07-30: rollups and per-model cost are already live (`totals` 121 requests / 118,355 tok / $0.2367, real `byModel`), so the row was wrong about what is missing. The defect is that `byKey` reports 0 for every key and `bySubject` is empty — budgets exist (`budgetUsd:120`) that no usage can consume, which is what makes "budget enforced" vacuous. Attribute each model call to its key/subject at spend-record time, then assert `byKey` sums to `totals`. Also decide what a dollar means for on-prem inference (`localShare:1` with non-zero cost) and state the basis on screen. | B4.10 🔴 **Root cause confirmed:** the rollup code is right; `audit_events_v2` has no key column and every costed event is stamped `actor_id=service@offgrid.local`, so spend is real but anonymous. Fix at the WRITE path (schema + stamp the real actor), not in the rollup. | B4.10 🔴 |
