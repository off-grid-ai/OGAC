...
- **`OffGrid-2026`** — real console / Keycloak user password, in cleartext, for
  real accounts (`*@wednesday.is`). Appears in committed scripts:
  - `deploy/onprem/recover.sh:94` (printed in the health report)
  - `deploy/onprem/recover.sh:96` (used in a live token request as
    `username=mac@wednesday.is -d password=OffGrid-2026`)
...
- `scripts/metrics-dashboard.mjs:102`, `scripts/shoot-*.mjs`, `w1verify.mjs`:
  `mac@wednesday.is`, `mohammed.ali@wednesday.is`, `diksha.sharma@…`,
  `ali@wednesday.is` (~10 occurrences under `scripts/` + `w1verify.mjs`).
