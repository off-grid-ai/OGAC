...
test(
  'RPC cluster: cluster_head/rpc_port persist + derivePool/deriveClusters round-trip',
  {
  ...
  async (t) => {
    ...
    const { derivePool, deriveClusters, mapFleetNodeRow } = await import('@/lib/fleet');
    const { GET: getPool } = await import('@/app/api/v1/gateway/pool/route');
    ...
    const head = `${PFX}head`;
    const w1 = `${PFX}worker-a`;
    const w2 = `${PFX}worker-b`;
    await db.insert(fleetNodes).values([
      {
        name: head,
        host: 'tint-head.local',
        port: 8439,
        role: 'gateway',
        kind: 'chat',
        model: 'qwythos-9b-1m',
    ...
    const rows = (
      await db
        .select()
        .from(fleetNodes)
        .where(like(fleetNodes.name, `${PFX}%`))
    ).map(mapFleetNodeRow);
    ...
    assert.deepEqual(
      pool.map((p) => p.name),
      [head],
      'only the head is routed; workers are bonded, not routed',
    );
    assert.equal(pool[0].port, 8439);

    // ── ROUTE contract: the real route reads the same DB and delegates through the canonical mapper.
    const response = await getPool(new Request('http://console.test/api/v1/gateway/pool') as never);
    assert.equal(response.status, 200);
    const routePayload = (await response.json()) as { pool: Array<{ name: string; port: number }> };
    const routedFixtureNodes = routePayload.pool.filter((entry) => entry.name.startsWith(PFX));
    assert.deepEqual(
      routedFixtureNodes.map(({ name, port }) => ({ name, port })),
      [{ name: head, port: 8439 }],
    );
    ...
    await db
      .update(fleetNodes)
      .set({ clusterHead: null, model: 'gemma-4-e4b' })
      .where(eq(fleetNodes.name, w1));
    const rows2 = (
      await db
        .select()
        .from(fleetNodes)
    ...
    assert.deepEqual(
      after.clusters[0].workers.map((w) => w.name),
      [w2],
      'the freed worker left the cluster',
    );
    assert.deepEqual(
      after.standalone.map((n) => n.name),
      [w1],
      'the freed worker is now standalone',
    );
  },
);
