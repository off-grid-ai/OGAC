...
const suite = {
  name: 'kyc', description: 'KYC quality', expectations: [expectation], version: 1,
  createdAt: '2026-07-23T00:00:00.000Z', updatedAt: '2026-07-23T00:00:00.000Z',
};
const run = {
  id: 'run_1', suiteName: 'kyc', suiteVersion: 1, success: true, evaluated: 1, failed: 0,
  ...
  engine: 'great-expectations', engineVersion: '1.8.0', dataSourceId: null, assetName: null,
};

async function startServer(options: { operations?: Record<string, boolean>; malformed?: boolean } = {}) {
  const requests: Array<{ method: string; url: string; headers: http.IncomingHttpHeaders; body: unknown }> = [];
  const operations = options.operations ?? {
    profile: true, validate: true, 'suite.list': true, 'suite.read': true,
    'suite.create': true, 'suite.update': true, 'suite.delete': true, 'history.list': true,
  };
  const server = http.createServer((req, res) => {
    ...
    req.on('end', () => {
      ...
      if (req.url === '/v1/capabilities') {
        res.end(JSON.stringify({ status: 'ok', engine: 'great-expectations', engineVersion: '1.8.0', operations }));
      } else if (options.malformed) {
        res.end(JSON.stringify({ unexpected: true }));
      } else if (req.method === 'POST' && req.url === '/v1/profiles') {
        res.end(JSON.stringify({
          dataSourceId: 'warehouse', assetName: 'customers', profiledAt: '2026-07-23T00:00:00.000Z', sampledRows: 2,
          columns: [{ name: 'pan', inferredType: 'string', rowCount: 2, nullCount: 0, distinctCount: 2, min: null, max: null }],
        }));
        ...
        res.statusCode = 201; res.end(JSON.stringify(suite));
        ...
        res.statusCode = 201; res.end(JSON.stringify(run));
...
test('real HTTP adapter drives profile, suite CRUD, validation, and history with server-owned tenant auth', async () => {
  ...
  const adapter = createGreatExpectationsLifecycleAdapter({ baseUrl: remote.url, token: 'service-secret' });
  try {
    ...
    assert.equal((await adapter.profile(context, { dataSourceId: 'warehouse', assetName: 'customers', sampleLimit: 100 })).ok, true);
    assert.equal((await adapter.listSuites(context)).ok, true);
    assert.equal((await adapter.getSuite(context, 'kyc')).ok, true);
    assert.equal((await adapter.createSuite(context, { name: 'kyc', description: 'KYC quality', expectations: [expectation] })).ok, true);
    assert.equal((await adapter.updateSuite(context, 'kyc', { expectedVersion: 1, description: 'tightened' })).ok, true);
    assert.equal((await adapter.deleteSuite(context, 'kyc', 2)).ok, true);
    assert.equal((await adapter.runValidation(context, { suiteName: 'kyc', batch: { kind: 'inline', rows: [{ pan: 'ABCDE1234F' }] } })).ok, true);
    assert.equal((await adapter.history(context, { suiteName: 'kyc', dataSourceId: 'warehouse', limit: 25, cursor: 'cursor_1' })).ok, true);

    const productRequests = remote.requests.filter((request) => request.url !== '/v1/capabilities');
    assert.equal(productRequests.length, 8);
    assert.equal(productRequests.every((request) => request.headers.authorization === 'Bearer service-secret'), true);
    assert.equal(productRequests.every((request) => request.headers['x-offgrid-org-id'] === context.orgId), true);
    assert.equal(productRequests.every((request) => request.headers['x-offgrid-actor'] === context.actor), true);
...
test('adapter does not call an operation the remote manifest did not advertise', async () => {
  ...
  const adapter = createGreatExpectationsLifecycleAdapter({ baseUrl: remote.url, token: 'service-secret' });
  try {
    const result = await adapter.profile(context, { dataSourceId: 'warehouse', assetName: 'customers', sampleLimit: 100 });
    assert.equal(result.ok, false);
    if (!result.ok) assert.equal(result.kind, 'unavailable');
    assert.deepEqual(remote.requests.map((request) => request.url), ['/v1/capabilities']);
  } finally {
    await remote.close();
  }
});
...
test('adapter fails closed before network I/O when tenant or service auth is missing', async () => {
  ...
  try {
    ...
    const invalidTenant = createGreatExpectationsLifecycleAdapter({ baseUrl: remote.url, token: 'service-secret' });
