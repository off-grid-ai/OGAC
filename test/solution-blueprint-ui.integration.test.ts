...
test('the rendered blueprint create journey exposes requirements, outcome, ROI and proof controls', () => {
  ...
  for (const label of [
    'Blueprint name', 'Business owner', 'Required data domains', 'Governed pipeline',
    'Outcome contract', 'Baseline', 'Target', 'Measured', 'Justifiable ROI', 'Annual benefit',
    'Implementation cost', 'Proof version', 'Proven deployments',
  ]) assert.match(html, new RegExp(label));
  assert.match(html, /Create blueprint/);
});

test('deployment creation visibly binds a blueprint to a canonical App', () => {
  const html = renderToStaticMarkup(createElement(DeploymentForm, {
