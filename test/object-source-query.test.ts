...
const scope: ObjectAccessScope = {
  domainId: 'dom_claims', domainLabel: 'Claim evidence', connectorId: 'con_s3',
  bucket: 'claims-archive', prefix: 'approved/',
};

function detail(overrides: Partial<ObjectDetail> = {}): ObjectDetail {
  return {
    bucket: scope.bucket, key: 'approved/case-1.json', size: 11,
    contentType: 'application/json', lastModified: '2026-07-23T00:00:00.000Z', etag: 'etag-1',
    metadata: {}, ...overrides,
  };
}
...
test('a mixed-prefix listing fails whole before any out-of-scope key can be selected', () => {
  const result = selectListedObjectKeys(scope, {
    prefix: 'approved/', folders: [], nextToken: null,
    objects: [
...
test('materialization retains full provenance and rejects metadata drift or unsafe content', () => {
  ...
  const row = materializeObjectSourceRow({ scope, detail: detail(), bytes, getContentType: 'application/json' });
  ...
  assert.equal(materializeObjectSourceRow({ scope, detail: detail({ size: 12 }), bytes, getContentType: 'application/json' }).ok, false);
  assert.equal(materializeObjectSourceRow({ scope, detail: detail({ contentType: 'application/pdf' }), bytes, getContentType: 'application/pdf' }).ok, false);
  assert.equal(materializeObjectSourceRow({ scope, detail: detail({ size: MAX_OBJECT_SOURCE_BYTES + 1 }), bytes, getContentType: 'application/json' }).ok, false);
  assert.equal(materializeObjectSourceRow({ scope, detail: detail({ size: 8 }), bytes: Buffer.from('not-json'), getContentType: 'application/json' }).ok, false);
});

test('aggregate content cannot exceed one MiB even when every object is individually valid', () => {
  const row = {
    key: 'one.txt', contentType: 'text/plain', size: MAX_OBJECT_SOURCE_BYTES,
    content: '', provenance: { connectorId: 'c', domainId: 'd', bucket: 'b', key: 'k', etag: 'e', lastModified: '', sha256: 'a'.repeat(64) },
  };
