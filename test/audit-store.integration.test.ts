...
test('persistAuditEvent writes a canonical, attributed row to audit_events_v2', { skip: dbUp ? false : SKIP_MESSAGE }, async (t) => {
  ...
  const ev = await persistAuditEvent({
    actor: { type: 'user', id: 'int-tester@wednesday.is', label: 'Int Tester' },
    org: ORG,
    project: 'proj-int',
    action: 'chat.send',
    resource: 'conv:int-1',
  ...
  assert.equal(ev.actor.id, 'int-tester@wednesday.is');
  ...
  assert.equal(row.actor_id, 'int-tester@wednesday.is');
