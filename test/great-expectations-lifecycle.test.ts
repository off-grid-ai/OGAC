...
test('suite create and update normalize valid lifecycle input', () => {
  const create = parseSuiteDraft({
    name: 'customer-quality.v1',
    description: '  PAN rules ',
...
test('suite validation rejects traversal, fake expectation types, empty updates, and unbounded suites', () => {
  ...
  assert.equal(
    parseSuiteDraft({ name: 'suite', expectations: [{ type: 'sql', kwargs: {} }] }).ok,
    false,
  );
  assert.equal(
    parseSuiteDraft({ name: 'suite', expectations: [{ type: 'expect_valid_name', kwargs: [] }] })
      .ok,
    false,
  );
  assert.equal(parseSuiteDraft({ name: 'suite', expectations: [] }).ok, false);
  assert.equal(
    parseSuiteDraft({ name: 'suite', expectations: Array.from({ length: 201 }, () => expectation) })
      .ok,
    false,
  );
...
test('profile parsing requires a governed source/asset and applies a bounded default sample', () => {
  assert.deepEqual(
    parseProfileRequest({ dataSourceId: 'warehouse', assetName: 'customers' }).value,
    {
  ...
  assert.equal(
    parseProfileRequest({ dataSourceId: 'warehouse', assetName: 'customers', sampleLimit: 100_001 })
      .ok,
    false,
  );
  assert.equal(
    parseProfileRequest({ dataSourceId: 'warehouse', assetName: 'customers', sampleLimit: '100' })
      .ok,
    false,
  );
});
...
test('validation request accepts bounded inline rows or an asset, never malformed batches', () => {
  ...
  assert.deepEqual(
    parseValidationRequest({
      ...
      batch: { kind: 'asset', dataSourceId: 'warehouse', assetName: 'customers' },
    }).value,
    {
      suiteName: 'kyc',
      batch: { kind: 'asset', dataSourceId: 'warehouse', assetName: 'customers', limit: 1_000 },
      idempotencyKey: 'run_1',
    },
  );
  assert.equal(
    parseValidationRequest({ suiteName: 'kyc', batch: { kind: 'inline', rows: [null] } }).ok,
    false,
  );
  assert.equal(
    parseValidationRequest({ suiteName: 'kyc', batch: { kind: 'query', sql: 'select *' } }).ok,
    false,
  );
  assert.equal(
    parseValidationRequest({ suiteName: '../kyc', batch: { kind: 'inline', rows: [] } }).ok,
    false,
  );
});
...
test('history query is bounded and cursor-safe', () => {
  ...
  assert.equal(
    parseHistoryQuery({ dataSourceId: 'warehouse', limit: 25, cursor: 'eyJpZCI6IjEifQ' }).ok,
    true,
  );
});
...
test('manifest parsing never invents support and distinguishes real GX from compatibility mode', () => {
  ...
  assert.equal(
    Object.values(compatibility.operations).every((supported) => !supported),
    true,
  );
