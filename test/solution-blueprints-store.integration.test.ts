...
test('solution library and deployment bindings are CRUD-capable and org isolated', {
  ...
  const app = await createApp(ORG_A, 'owner@test.local', {
    title: 'Collections deployment', summary: 'Tenant app', visibility: 'private',
    trigger: { kind: 'on-demand' },
    steps: [{ id: 's1', kind: 'agent', label: 'Prioritise', inlineAgent: { systemPrompt: 'Prioritise delinquency cases.' } }],
    edges: [],
  });
  appId = app.id;

  const deployment = await store.createSolutionDeployment(ORG_A, {
    blueprintId: created.id, appId, status: 'active', evidenceLinks: ['/governance/evidence'],
  });
