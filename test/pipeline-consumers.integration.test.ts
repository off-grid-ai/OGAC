...
test('app pipeline binding persists + listAppsByPipeline scopes by pipeline and org', { skip: dbUp ? false : SKIP_MESSAGE }, async (t) => {
  const { createApp, updateApp, getApp, listAppsByPipeline, deleteApp, listApps } = await import(
    '@/lib/apps-store'
  );
  ...
  const mkAgent = (title: string, pipelineId: string | null) => ({
    ...
    steps: [{ id: 'a', label: title, kind: 'agent' as const, inlineAgent: { systemPrompt: 'x' } }],
    edges: [],
    pipelineId,
  });

  t.after(async () => {
    for (const org of [ORG, OTHER]) for (const a of await listApps(org)) await deleteApp(a.id, org);
  });

  const a1 = await createApp(ORG, 'owner@x.io', mkAgent('Bound A', 'pl_alpha'));
  const a2 = await createApp(ORG, 'owner@x.io', mkAgent('Bound A2', 'pl_alpha'));
  const b1 = await createApp(ORG, 'owner@x.io', mkAgent('Bound B', 'pl_beta'));
  const unbound = await createApp(ORG, 'owner@x.io', mkAgent('Unbound', null));
  // Same pipeline id but a DIFFERENT org — must NOT leak into ORG's results.
  await createApp(OTHER, 'owner@x.io', mkAgent('Other-org Alpha', 'pl_alpha'));

  // Persisted round-trip.
  assert.equal((await getApp(a1.id, ORG))?.pipelineId, 'pl_alpha');
  assert.equal((await getApp(unbound.id, ORG))?.pipelineId, null);

  const alpha = await listAppsByPipeline('pl_alpha', ORG);
  assert.deepEqual(new Set(alpha.map((a) => a.id)), new Set([a1.id, a2.id]), 'only ORG pl_alpha apps');
  const beta = await listAppsByPipeline('pl_beta', ORG);
  assert.deepEqual(beta.map((a) => a.id), [b1.id]);

  // Re-point via updateApp, then it moves buckets.
  await updateApp(a1.id, ORG, { pipelineId: 'pl_beta' });
  assert.deepEqual(
    new Set((await listAppsByPipeline('pl_alpha', ORG)).map((a) => a.id)),
    new Set([a2.id]),
    'a1 left pl_alpha',
  );
  assert.deepEqual(
    new Set((await listAppsByPipeline('pl_beta', ORG)).map((a) => a.id)),
    new Set([a1.id, b1.id]),
    'a1 joined pl_beta',
  );
  ...
  assert.equal((await listAppsByPipeline('pl_beta', ORG)).some((a) => a.id === a1.id), false);
});

test('chat project pipeline binding persists + listProjectsByPipeline scopes', { skip: dbUp ? false : SKIP_MESSAGE }, async (t) => {
  const { createProject, getProjectBinding, listProjectsByPipeline, deleteProject } = await import(
    '@/lib/chat'
  );
  ...
  assert.deepEqual(c2.map((p) => p.id), [p3]);
  assert.equal((await listProjectsByPipeline('pl_none')).length, 0);
});

test('org chat-binding governance round-trips + gates disallowed picks', { skip: dbUp ? false : SKIP_MESSAGE }, async (t) => {
