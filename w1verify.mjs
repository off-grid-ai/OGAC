...
if (await p.$('text=Continue with Keycloak')) { await p.click('text=Continue with Keycloak'); await p.waitForSelector('#username',{timeout:15000}); await p.fill('#username','admin@getoffgridai.co'); await p.fill('#password','changeme'); await Promise.all([p.waitForNavigation({waitUntil:'networkidle'}), p.click('#kc-login')]); }
