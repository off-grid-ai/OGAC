...
`RESEND_API_KEY` (send-only key from mobile/.env.keygen) + `RESEND_FROM="Off Grid AI <noreply@offgridmobileai.co>"` set in server `.env.local` (NOT git). The Resend account's verified sending domain is **offgridmobileai.co** (getoffgridai.co is NOT authorized on this key; `noreply@`/`hello@`/`support@offgridmobileai.co` work). Real invite emails (branded, with accept link) sent + delivered to mac@wednesday.is + mohammed.ali@wednesday.is via the proper createInvite→sendInviteEmail flow. Fixed a real bug: the Resend sink 422'd on an empty tag value — now drops tags unless name AND value survive sanitizing (`email-resend.ts`, committed 8e206c4).
