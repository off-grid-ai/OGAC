...
# ── 4. health report ──────────────────────────────────────────────────────────
hd "HEALTH REPORT"
echo "One URL: http://${SERVER_IP}   (users: *@example.com / changeme)"
...
tok=$(curl -s --max-time 8 "http://${SERVER_IP}:8080/realms/offgrid/protocol/openid-connect/token" -d grant_type=password -d client_id=offgrid-console -d client_secret=offgrid-dev-keycloak-secret -d username=mac@example.com -d password=changeme -d scope=openid 2>/dev/null | grep -c access_token)
